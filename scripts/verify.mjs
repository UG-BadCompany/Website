...
for(const route of ['/install-status','/install/health','/install/finish','/install/integration-status','/system/integration-status','/auth/magic-link','/workflow/transition','/health']) if(!api.includes(route)) failures.push(`Missing route ${route}`);
...
for(const table of ['platform_installation','company_settings','homepage_settings','app_users','roles','permissions','role_permissions','user_roles','workspace_access','module_registry','module_settings','service_categories','customers','estimate_requests','quotes','work_orders','inventory_items','inventory_transactions','invoices','payments','files','workflow_events','audit_logs','magic_link_tokens','platform_secret_settings']) if(!db.includes(`create table if not exists ${table}`)) failures.push(`Missing table ${table}`);
