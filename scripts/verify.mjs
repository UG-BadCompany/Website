...
assert(db.indexOf('const value = getConnectionString();') < db.indexOf('for (const name of databaseUrlEnvNames)'), 'database loader must try getConnectionString() before environment fallbacks');
for (const name of ['NETLIFY_DB_URL','NETLIFY_DATABASE_URL','DATABASE_URL','POSTGRES_URL','POSTGRES_PRISMA_URL','POSTGRES_URL_NON_POOLING','NEON_DATABASE_URL']) assert(db.includes(name), `database loader missing fallback ${name}`);
...
const shouldExerciseDatabase = Boolean(process.env.NETLIFY || process.env.NETLIFY_DB_URL || process.env.NETLIFY_DATABASE_URL || process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_PRISMA_URL || process.env.POSTGRES_URL_NON_POOLING || process.env.NEON_DATABASE_URL);
