window.TAAI={
  ...
  draftQuote:async(payload)=>{const response=await TAApi.post('/api/admin/quote-draft',payload);if(response?.draft&&window.TAQuotes?.normalizeAiDraft)response.draft=TAQuotes.normalizeAiDraft(response.draft);if(response?.result&&window.TAQuotes?.normalizeAiDraft)response.result=TAQuotes.normalizeAiDraft(response.result);return response},
