(() => {
  const fallback = {
    ...
    sidebarBackgroundColor: '#0f172a', sidebarTextColor: '#e5edf7', sidebarActiveColor: '#2563eb', sidebarBorderColor: '#1e293b', sidebarHoverColor: '#1e293b', mobileNavBackgroundColor: '#0f172a', mobileNavActiveColor: '#2563eb',
  ...
  async function load() { try { const data = await window.TAApi.get('/.netlify/functions/company-settings'); const company = norm(data.company || data.settings || {}); apply(company); return company; } catch (e) { apply(fallback); return fallback; } }
