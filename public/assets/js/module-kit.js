(() => {
  ...
  const normalizeRoot = (context) => window.TAModuleRoot?.resolve?.(context) || document.getElementById('module-root') || document.querySelector('[data-module-root]');
  ...
  const renderRecords = (mountRoot, config, data, allRecords) => {
    const query = mountRoot.querySelector('[data-module-search]')?.value || '';
    const tab = mountRoot.querySelector('[data-module-tab].active')?.dataset.moduleTab || 'all';
    ...
    const list = mountRoot.querySelector('[data-module-records]');
    if (!records.length) {
    ...
    list.querySelectorAll('.module-record-card').forEach((card, index) => card.querySelectorAll('[data-module-action]').forEach((control) => control.onclick = () => config.onRecordAction ? config.onRecordAction(control.dataset.moduleAction, { root: mountRoot, api: window.TAApi, config, record: records[index], records, data, router: window.TADashboardRouter }) : runGenericAction(control, control.dataset.moduleAction, { root: mountRoot, api: window.TAApi, config, record: records[index], records, data })));
  };
  ...
  const mount = async (context = {}, config) => {
    const { api, user, company, router, workspace } = context;
    const mountRoot = normalizeRoot(context);
    ...
    mountRoot.querySelectorAll('[data-module-action]').forEach((control) => control.onclick = () => effective.onAction ? effective.onAction(control.dataset.moduleAction, { root: mountRoot, api, user, company, data, records, config:effective }) : runGenericAction(control, control.dataset.moduleAction, { root: mountRoot, api, user, company, data, records, config:effective, router, workspace }));
    mountRoot.querySelectorAll('[data-module-tab]').forEach((tab) => tab.onclick = () => { mountRoot.querySelectorAll('[data-module-tab]').forEach((item) => item.classList.remove('active')); tab.classList.add('active'); renderRecords(mountRoot, effective, data, records); });
    mountRoot.querySelector('[data-module-search]')?.addEventListener('input', () => renderRecords(mountRoot, effective, data, records));
    renderRecords(mountRoot, effective, data, records);
  };
