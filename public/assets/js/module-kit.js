(() => {
  ...
  const stateCard = (type, title, body) => `<article class="module-state module-state-${type}"><h3>${escapeHtml(title)}</h3><p>${escapeHtml(body)}</p></article>`;
  ...
  const renderMetrics = (metrics, data, records) => `<div class="module-stat-grid">${metrics.map((metric) => {
    ...
    return `<article class="module-stat"><span>${escapeHtml(metric.icon || '📌')}</span><strong>${escapeHtml(formatted)}</strong><small>${escapeHtml(metric.label)}</small></article>`;
  }).join('')}</div>`;
  ...
  const mount = async ({ root, api, user, company, router, workspace }, config) => {
    ...
    root.innerHTML = `<section class="module-shell stack"><div class="module-hero card"><div><p class="eyebrow">${escapeHtml(effective.workspaceLabel || titleize(workspace || effective.role))}</p><h2>${escapeHtml(effective.icon || '📌')} ${escapeHtml(effective.title)}</h2><p>${escapeHtml(effective.description || 'Operational tools for this workspace.')}</p></div><div class="module-toolbar">${(effective.actions || []).map((action) => button(action.label || action, action.action || action, action.primary ? '' : 'secondary')).join('')}</div></div>${stateCard('loading', 'Loading module data', 'Checking available endpoints and preparing your workspace...')}</section>`;
