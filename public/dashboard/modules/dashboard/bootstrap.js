    (async () => {
      ...
      const renderAdminWorkOrderSummary = (request, assignments = [], quote = null) => {
        ...
        const workflowSteps = [
          ...
          { key: 'waiting_payment', label: 'Ready to invoice' },
      ...
      const getInvoiceReadinessBlocks = (request, reservations = [], movements = []) => {
        const openReservations = reservations.filter((item) => ['reserved', 'partially_used'].includes(item.status));
        const blocks = [];
        if (request && !['accepted', 'scheduled', 'in_progress', 'pending_review', 'waiting_payment', 'completed'].includes(request.status || '')) blocks.push('Client quote/request is not accepted yet.');
        if (request && !['pending_review', 'waiting_payment', 'completed'].includes(request.status || '')) blocks.push('Labor is not marked complete by the worker yet.');
        if (request && request.status === 'pending_review') blocks.push('Admin completion review is still pending.');
        if (openReservations.length) blocks.push(`${openReservations.length} reserved material line${openReservations.length === 1 ? '' : 's'} still need use, release, or keep-reserved reason.`);
        if (reservations.length && !movements.length) blocks.push('Material reservations have no movement history yet.');
        return blocks;
      };

      const renderAdminWorkOrderMaterials = ({ reservations = [], movements = [], request = null } = {}) => {
        ...
        if (readiness) {
          const blocks = getInvoiceReadinessBlocks(request, reservations, movements);
          readiness.dataset.readyToInvoice = blocks.length ? 'false' : 'true';
          readiness.innerHTML = `
            <strong>Invoice/payment readiness: ${blocks.length ? 'Not ready' : 'Ready'}</strong>
            <p>${blocks.length ? 'Blocking reasons must be resolved before invoice closeout.' : 'Labor, admin review, and reserved materials are resolved. Invoice can include material usage totals.'}</p>
            ${blocks.length ? `<ul>${blocks.map((block) => `<li>${escapeHtml(block)}</li>`).join('')}</ul>` : ''}
            <div class="admin-request-meta"><span>Material charge total ${escapeHtml(formatMoney(materialChargeTotal * 100))}</span><span>${movements.length} movement${movements.length === 1 ? '' : 's'}</span><span>${openReservations.length} open reservation${openReservations.length === 1 ? '' : 's'}</span></div>
          `;
        }
      };
      ...
      const loadAdminWorkOrderInventory = async (jobRequestId) => {
        ...
        try {
          ...
          renderAdminWorkOrderMaterials({ reservations: currentAdminInventoryReservations, movements: currentAdminInventoryMovements, request: currentAdminRequests.get(jobRequestId) || null });
          ...
          renderAdminWorkOrderMaterials({ reservations: [], movements: [], request: currentAdminRequests.get(jobRequestId) || null });
      ...
      const renderWorkerJobMaterials = (assignment = {}) => {
        const jobRequestId = assignment.jobRequest?.id || '';
        const reservations = currentWorkerInventoryReservations.filter((entry) => String(entry.jobRequestId || '') === String(jobRequestId));
        if (!reservations.length) return '<p class="session-status">No reserved job materials assigned yet. Use the stock request flow if more material is needed.</p>';
        return reservations.map((entry) => {
          const unused = Math.max(0, Number(entry.reservedQuantity || 0) - Number(entry.usedQuantity || 0));
          return `
            <article class="admin-assignment" data-worker-job-material>
              <span class="admin-assignment-badge">${escapeHtml(entry.status || 'reserved')}</span>
              <strong>${escapeHtml(entry.itemName || 'Reserved material')}</strong>
              <div class="admin-assignment-meta"><span>Reserved ${Number(entry.reservedQuantity || 0)}</span><span>Used ${Number(entry.usedQuantity || 0)}</span><span>Remaining ${unused}</span><span>${escapeHtml(entry.itemSku || 'No SKU')}</span></div>
              ${unused > 0 ? `<div class="client-quote-actions"><input aria-label="Quantity for ${escapeHtml(entry.itemName || 'material')}" data-worker-material-quantity value="${unused}" type="number" min="0" step="0.01"><button class="btn btn-soft" type="button" data-worker-material-use data-item-id="${escapeHtml(entry.itemId)}" data-job-request-id="${escapeHtml(jobRequestId)}">Mark used</button><button class="btn btn-soft" type="button" data-worker-material-release data-item-id="${escapeHtml(entry.itemId)}" data-job-request-id="${escapeHtml(jobRequestId)}">Release unused</button></div>` : '<p class="session-status">Material line is resolved for closeout.</p>'}
            </article>
          `;
        }).join('');
      };
      ...
      const renderWorkerJobCard = (assignment) => {
        ...
        <article class="client-quote worker-job" data-worker-status="${escapeHtml(statusTone)}">
          ...
          <div class="admin-assignment-list" data-worker-job-materials="${escapeHtml(assignment.jobRequest?.id || '')}">
            ${renderWorkerJobMaterials(assignment)}
          </div>
          <div class="client-quote-actions">
          ...
          <form class="client-request-form" data-worker-job-form data-assignment-id="${escapeHtml(assignment.id)}" data-job-request-id="${escapeHtml(assignment.jobRequest?.id || '')}" hidden>
            ...
            <div class="client-request-form-actions"><button class="btn btn-primary" type="submit">Save job update</button><button class="btn btn-soft" type="button" data-worker-mark-complete>Mark job complete</button><span data-worker-job-form-status aria-live="polite"></span></div>
          </form>
...
Additional info from client: ${payload.additionalInfo}` : '';
      ...
      const hydrateAdminQuoteForm = (quote = null, request = {}) => {
        ...
        const requestReadout = quoteForm.querySelector('[data-admin-quote-request-readout]');
        const customerReadout = quoteForm.querySelector('[data-admin-quote-customer-readout]');
        const confidenceReadout = quoteForm.querySelector('[data-admin-quote-confidence-readout]');
        const assumptionsField = quoteForm.querySelector('[data-admin-quote-assumptions]');
        const exclusionsField = quoteForm.querySelector('[data-admin-quote-exclusions]');
        const customerNotesField = quoteForm.querySelector('[data-admin-quote-customer-notes]');
        const internalNotesField = quoteForm.querySelector('[data-admin-quote-internal-notes]');
        const scores = quote?.confidenceScores || metadata.confidenceScores || request?.intakeAnalysis?.confidenceScores || {};
        const overall = Number(scores.overall || quote?.confidence || quote?.informationCompletenessScore || 0);
        if (customerReadout) customerReadout.textContent = `${quote?.clientName || request?.requesterName || request?.name || 'Customer'} · ${quote?.clientEmail || request?.requesterEmail || request?.email || 'No email'} · ${quote?.requesterPhone || request?.requesterPhone || request?.phone || 'No phone'}`;
        if (requestReadout) requestReadout.textContent = `${request?.serviceType || request?.service || quote?.serviceType || 'Service'} · ${request?.streetAddress || quote?.streetAddress || 'Address not provided'} ${request?.city || quote?.city || ''} · ${request?.description || quote?.requestDescription || quote?.summary || 'No request summary provided.'}`;
        if (confidenceReadout) confidenceReadout.innerHTML = [
          ['Overall Confidence', overall],
          ['Labor Confidence', Number(scores.labor || overall || 0)],
          ['Material Confidence', Number(scores.material || scores.materials || overall || 0)],
          ['Scope Confidence', Number(scores.scope || overall || 0)],
        ].map(([label, value]) => `<span>${escapeHtml(label)}<br><strong>${escapeHtml(value ? `${value}%` : '—')}</strong></span>`).join('');
        if (assumptionsField) assumptionsField.value = (metadata.assumptions || quote?.assumptions || []).join ? (metadata.assumptions || []).join('\n') : (metadata.assumptions || '');
        if (exclusionsField) exclusionsField.value = (metadata.exclusions || quote?.exclusions || []).join ? (metadata.exclusions || []).join('\n') : (metadata.exclusions || '');
        if (customerNotesField) customerNotesField.value = metadata.customerNotes || '';
        if (internalNotesField) internalNotesField.value = metadata.internalNotes || metadata.internal_technician_notes || '';
        if (aiStatus) {
          aiStatus.textContent = quote?.isRequestOnly ? 'Submitted estimate request needs an admin or AI draft. Admin can continue anyway.' : (quote ? `${quote.aiEnhanced ? 'AI Enhanced' : 'Manual quote'}${quote.fallbackUsed ? ' · Fallback Used' : ''} · ${quoteConfidenceLabel(quote)} confidence.` : 'Ready to generate an AI draft.');
        }
      };

      const quoteWorkflowState = (quote = {}) => {
        if (quote.isRequestOnly || quote.needsDraft || !(quote.quoteId || '') || String(quote.id || '').startsWith('request:')) {
          return {
            key: 'needs_draft',
            status: 'Needs Draft',
            title: 'Request Submitted',
            actions: [
              ['primary', 'Generate AI Draft', 'admin-quote-ai-card'],
              ['soft', 'Create Manual Draft', 'admin-create-manual-draft'],
              ['soft', 'Request Information', 'admin-request-info'],
            ],
          };
        }
        if (quote.status === 'sent' || quote.status === 'viewed') {
          return {
            ...
            title: 'Quote Sent',
            actions: [
              ['primary', 'View Quote', 'admin-edit-quote'],
              ['soft', 'Resend', 'admin-send-quote'],
              ['soft', 'Mark Accepted', 'admin-mark-accepted'],
              ['soft', 'Request Information', 'admin-request-info'],
            ],
          };
        }
        if (quote.status === 'accepted') {
          return {
            ...
            title: 'Accepted',
            actions: [
              ['primary', 'Convert To Work Order', 'admin-open-quote-work-order', quote.jobRequestId || quote.requestId || quote.request?.id || ''],
              ['soft', 'Schedule Job', 'admin-schedule-quote-job', quote.jobRequestId || quote.requestId || quote.request?.id || ''],
              ['soft', 'View Quote', 'admin-edit-quote'],
            ],
          };
        }
        return {
          ...
          title: 'Draft Created',
          actions: [
            ['primary', 'Review/Edit', 'admin-edit-quote'],
            ['soft', 'Save Draft', 'admin-save-quote-card'],
            ['soft', 'Request Information', 'admin-request-info'],
            ['soft', 'Send To Client', 'admin-send-quote'],
          ],
        };
      };

      const renderAdminQuoteAction = (quote, action) => {
        const [style, label, attr, overrideValue] = action;
        const value = overrideValue ?? quote.id ?? '';
        return `<button class="btn btn-${style}" type="button" data-${attr}="${escapeHtml(value)}">${escapeHtml(label)}</button>`;
      };

      const renderAdminQuoteCard = (quote = {}) => {
        const workflow = quoteWorkflowState(quote);
        const requestId = quote.jobRequestId || quote.requestId || quote.request?.id || '';
        const scores = quote.confidenceScores || {};
        const overall = Number(scores.overall || quote.confidence || quote.informationCompletenessScore || 0);
        const labor = Number(scores.labor || overall || 0);
        const material = Number(scores.material || scores.materials || overall || 0);
        const scope = Number(scores.scope || overall || 0);
        ...
        const customer = quote.clientName || quote.clientEmail || quote.request?.requesterName || quote.requesterName || 'Customer';
        ...
        const complete = Number(quote.informationCompletenessScore || overall || 0);
        const missing = Array.isArray(quote.missingInformation) ? quote.missingInformation : [];
        const optionalQuestions = Array.isArray(quote.optionalQuestions) ? quote.optionalQuestions : [];
        const lowWarning = complete && complete < 55 ? '<p class="session-status"><strong>Information Needed:</strong> More details may improve accuracy, but Admin can Generate AI Draft, Create Manual Draft, or Continue Anyway.</p>' : '';
        return `
          <article class="admin-request admin-quote-card" data-admin-quote-card="${escapeHtml(quote.id || '')}" data-admin-quote-status="${escapeHtml(workflow.key)}" data-admin-request-only="${quote.isRequestOnly || quote.needsDraft ? 'true' : 'false'}">
            <div class="admin-quote-state-row">
              <span class="admin-request-badge">${escapeHtml(workflow.title)}</span>
              <span class="admin-request-badge">${escapeHtml(workflow.status)}</span>
              ${quote.aiEnhanced ? '<span class="admin-request-badge">AI Draft</span>' : ''}
              ${quote.fallbackUsed ? '<span class="admin-request-badge">Fallback Used</span>' : ''}
            </div>
            ...
            <div class="admin-quote-confidence-row" aria-label="AI confidence scores">
              <span class="admin-quote-confidence-chip">Overall Confidence ${overall || '—'}%</span>
              <span class="admin-quote-confidence-chip">Labor Confidence ${labor || '—'}%</span>
              <span class="admin-quote-confidence-chip">Material Confidence ${material || '—'}%</span>
              <span class="admin-quote-confidence-chip">Scope Confidence ${scope || '—'}%</span>
            </div>
            <p>${escapeHtml(quote.summary || quote.requestDescription || quote.request?.description || 'Submitted estimate request needs review before a final quote is sent.')}</p>
            ${lowWarning}
            ${missing.length ? `<div class="admin-quote-missing-row"><span>Missing Information: ${escapeHtml(missing.slice(0, 4).map((item) => typeof item === 'string' ? item : item.prompt || item.label || '').join(' • '))}</span></div>` : ''}
            ${optionalQuestions.length ? `<div class="admin-quote-missing-row"><span>Optional questions ready: ${escapeHtml(optionalQuestions.slice(0, 3).map((item) => item.prompt || item.label || String(item)).join(' • '))}</span></div>` : ''}
            <div class="client-quote-actions">
              ${workflow.actions.map((action) => renderAdminQuoteAction(quote, action)).join('')}
              ${workflow.key === 'needs_draft' ? `<button class="btn btn-soft" type="button" data-admin-continue-anyway="${escapeHtml(quote.id || '')}">Continue Anyway</button>` : ''}
            </div>
          </article>`;
      };
      ...
      const loadAdminQuotes = async () => {
        ...
        const selectedFilter = document.querySelector('[data-admin-quote-status-filter]')?.value || 'needs_review';
        const statusFilter = selectedFilter === 'information_needed' ? 'needs_review' : selectedFilter;
        ...
        try {
          ...
          const quotes = selectedFilter === 'information_needed'
            ? loadedQuotes.filter((quote) => (quote.missingInformation || []).length || Number(quote.informationCompletenessScore || 100) < 70 || quote.needsDraft || quote.isRequestOnly)
            : loadedQuotes;
          const requestOnlyCount = quotes.filter((quote) => quote.isRequestOnly || quote.needsDraft).length;
          const informationNeededIntro = selectedFilter === 'information_needed' ? '<p class="session-status"><strong>Information Needed</strong> shows Customer, Service, Missing Information, Confidence Score, Completeness Score, and admin actions. Admin can Continue Anyway at any time.</p>' : '';
          list.innerHTML = quotes.length ? `${informationNeededIntro}${requestOnlyCount && selectedFilter !== 'information_needed' ? '<p class="session-status">Submitted estimate requests need review.</p>' : ''}${quotes.map(renderAdminQuoteCard).join('')}` : '<p class="session-status">No quotes match this filter.</p>';
      ...
      const selectAdminRequest = (requestId, options = {}) => {
        ...
        const reviewPanel = document.querySelector('[data-admin-completion-review-panel]');
        const reviewRequestId = document.querySelector('[data-admin-review-request-id]');
        const reviewEvidenceList = document.querySelector('[data-admin-completion-evidence-list]');
        ...
        if (reviewRequestId) reviewRequestId.value = request.id;
        ...
        const hasCompletionReview = ['pending_review', 'waiting_payment', 'completed'].includes(request.status || '');
        if (reviewPanel) reviewPanel.hidden = !hasCompletionReview;
        if (reviewEvidenceList) {
          const completionAssignments = requestAssignments.filter((assignment) => assignment.workerNotes || assignment.completionNotes || (assignment.completionPhotoNames || []).length || assignment.status === 'completed');
          reviewEvidenceList.innerHTML = completionAssignments.length ? completionAssignments.map((assignment) => `
            <article class="admin-assignment">
              <span class="admin-assignment-badge">${escapeHtml((assignment.status || 'completed').replaceAll('_', ' '))}</span>
              <strong>${escapeHtml(assignment.workerName || assignment.workerEmail || 'Assigned worker')}</strong>
              <p>${escapeHtml(assignment.completionNotes || assignment.workerNotes || 'Worker marked complete; review material closeout before invoicing.')}</p>
              <div class="admin-assignment-meta"><span>${escapeHtml((assignment.completionPhotoNames || []).length ? `${assignment.completionPhotoNames.length} evidence file(s)` : 'Before/after evidence placeholder ready')}</span><span>${escapeHtml(assignment.completionSubmittedAt ? formatDate(String(assignment.completionSubmittedAt).slice(0, 10)) : 'No completion timestamp')}</span></div>
            </article>
          `).join('') : '<p class="session-status">No worker completion notes or evidence submitted yet.</p>';
        }
        if (workOrderSummary) {
      ...
      const bindAdminQuoteWorkspaceActions = () => {
        ...
        workspace.addEventListener('click', async (event) => {
          ...
          const accepted = event.target.closest('[data-admin-mark-accepted]');
          const schedule = event.target.closest('[data-admin-schedule-quote-job]');
          try {
            ...
            if (accepted) {
              const status = document.querySelector('[data-admin-quotes-status]');
              if (status) status.textContent = 'Mark Accepted selected. Open the quote editor or work order pipeline to complete acceptance workflow.';
              return;
            }
            if (schedule) {
              window.taSetSidebarWorkspace?.('scheduling', { scroll: true, target: '#smart-schedule-suite' });
              return;
            }
            if (workOrder) {
      ...
      const bindAdminRequestActions = () => {
        ...
        const reviewForm = document.querySelector('[data-admin-completion-review-form]');
        if (reviewForm && !reviewForm.dataset.bound) {
          reviewForm.dataset.bound = 'true';
          reviewForm.addEventListener('submit', async (event) => {
            event.preventDefault();
            const formStatus = document.querySelector('[data-admin-completion-review-status]');
            const payload = Object.fromEntries(new FormData(reviewForm).entries());
            if (!payload.jobRequestId) {
              if (formStatus) formStatus.textContent = 'Open a work order before submitting review.';
              return;
            }
            if (formStatus) formStatus.textContent = 'Submitting completion review…';
            try {
              const response = await fetch('/api/admin/work-orders/review', {
                method: 'POST',
                headers: { accept: 'application/json', 'content-type': 'application/json' },
                body: JSON.stringify(payload),
              });
              const result = await response.json().catch(() => ({}));
              if (!response.ok || !result.ok) throw new Error(result.message || 'Could not submit completion review.');
              if (formStatus) formStatus.textContent = result.invoiceReadiness?.ready ? 'Completion approved. Ready to invoice.' : 'Completion review saved; invoice blockers remain.';
              await loadAdminRequests(payload.jobRequestId);
              await loadAdminActivity();
            } catch (error) {
              if (formStatus) formStatus.textContent = error.message;
      ...
      const bindWorkerJobActions = () => {
        ...
        panel.addEventListener('click', async (event) => {
          ...
          const materialButton = event.target.closest('[data-worker-material-use], [data-worker-material-release]');
          if (materialButton) {
            event.preventDefault();
            const card = materialButton.closest('[data-worker-status]');
            const formStatus = card?.querySelector('[data-worker-job-form-status]') || document.querySelector('[data-worker-jobs-status]');
            const quantityInput = materialButton.closest('[data-worker-job-material]')?.querySelector('[data-worker-material-quantity]');
            const quantity = Number(quantityInput?.value || 0);
            if (!quantity || quantity <= 0) {
              if (formStatus) formStatus.textContent = 'Enter a material quantity greater than zero.';
              return;
            }
            const action = materialButton.matches('[data-worker-material-use]') ? 'use' : 'release';
            // Phase53 endpoints: /api/worker/inventory/use /api/worker/inventory/release
            if (formStatus) formStatus.textContent = action === 'use' ? 'Marking reserved material used…' : 'Releasing unused reserved material…';
            try {
              const response = await fetch(`/api/worker/inventory/${action}`, {
                method: 'POST',
                headers: { accept: 'application/json', 'content-type': 'application/json' },
                body: JSON.stringify({ itemId: materialButton.dataset.itemId, jobRequestId: materialButton.dataset.jobRequestId, quantity, note: action === 'use' ? 'Worker used reserved material on job closeout' : 'Worker released unused reserved material during closeout' }),
              });
              const result = await response.json().catch(() => ({}));
              if (!response.ok || !result.ok) throw new Error(result.message || 'Could not update reserved material.');
              if (formStatus) formStatus.textContent = action === 'use' ? 'Material usage recorded.' : 'Unused material released.';
              await loadWorkerJobs();
            } catch (error) {
              if (formStatus) formStatus.textContent = error.message;
            }
          ...
          const completeButton = event.target.closest('[data-worker-mark-complete]');
          if (completeButton) {
            event.preventDefault();
            const form = completeButton.closest('[data-worker-job-form]');
            const formStatus = form?.querySelector('[data-worker-job-form-status]');
            if (!form) return;
            const payload = Object.fromEntries(new FormData(form).entries());
            payload.assignmentId = form.dataset.assignmentId;
            payload.status = 'completed';
            if (formStatus) formStatus.textContent = 'Marking job complete for admin review…';
            try {
              const response = await fetch('/api/worker/jobs/complete', {
                method: 'POST',
                headers: { accept: 'application/json', 'content-type': 'application/json' },
                body: JSON.stringify(payload),
              });
              const result = await response.json().catch(() => ({}));
              if (!response.ok || !result.ok) throw new Error(result.message || 'Could not mark job complete.');
              if (formStatus) formStatus.textContent = 'Job marked complete. Admin review is next.';
              await loadWorkerJobs();
            } catch (error) {
              if (formStatus) formStatus.textContent = error.message;
            }
      ...
      const loadWorkerJobs = async () => {
        ...
        try {
          ...
          const inventoryResult = await fetch('/api/worker/inventory', { headers: { accept: 'application/json' } })
