(() => {
  ...
  window.TAModules.register({
    ...
    async mount(context = {}) { const mountRoot = window.TAModuleRoot.resolve(context); if (!mountRoot?.querySelector) throw new TypeError('Module root element was not found.'); const root = mountRoot;
      mountRoot.innerHTML = `<section class="module-page stack ai-troubleshooting-2"><div class="module-hero module-header card"><div><p class="eyebrow">Worker AI Tools</p><h2 class="module-title">🧰 AI Troubleshooting 2.0</h2><p class="module-description">Research-backed diagnostics using trade, manufacturer, model/serial, age, complaint, symptoms, fault codes, photos, and technician mode.</p></div></div><div class="trouble-layout">${formMarkup}<div id="trouble-output" class="module-card card stack trouble-output"><h3>Diagnostic output</h3><p class="module-empty">Run the assistant to research manufacturer/model/error data, analyze findings, cache successful lookups, and build a technician-grade workflow.</p></div></div></section>`;
      mountRoot.querySelector('#trouble').onsubmit = async (event) => {
        event.preventDefault();
        const out = mountRoot.querySelector('#trouble-output');
