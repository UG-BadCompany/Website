(() => {
  ...
  window.TAModules.register({ id:'admin.module-manager', role:'admin', title:'Module Manager', icon:'🧩', permissions:['settings.manage'], async mount(context = {}) { const { api, router } = context; const mountRoot = window.TAModuleRoot.resolve(context); if (!mountRoot?.querySelector) throw new TypeError('Module root element was not found.'); const root = mountRoot;
    let data = { modules: [] };
