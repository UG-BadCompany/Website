  ...
  window.TAModules.register({
    ...
    async mount({ root, api, router }) {
      ...
      const roleCard = (role) => {
        ...
        return `<article class="role-card module-card ${isProtected ? 'protected' : ''}">
          ...
          <div class="action-row">
            <button class="btn secondary" type="button" data-edit-role="${escapeHtml(role.id)}">Edit Role</button>
            <button class="btn secondary" type="button" data-clone-role="${escapeHtml(role.id)}">Clone Role</button>
            ${!role.isSystem && role.key !== 'owner' ? `<button class="btn danger" type="button" data-delete-role="${escapeHtml(role.id)}">Delete Custom Role</button>` : `<button class="btn secondary" type="button" disabled>${role.key === 'owner' ? 'Owner Protected' : 'Protected'}</button>`}
          </div>
        </article>`;
      };
      ...
      const editor = (role = { permissions: [] }) => {
        ...
        root.querySelector('#copy-permissions').addEventListener('change', (event) => { const source = data.roles.find((item) => String(item.id) === event.target.value); if (!source) return; const sourcePermissions = new Set(asArray(source.permissions)); root.querySelectorAll('[name="permissions"]').forEach((input) => { input.checked = sourcePermissions.has(input.value); }); markUnsaved(); });
