(() => {
  ...
  window.TAModules.register({
    ...
    async mount({ root, api }) {
      ...
      const workspaceSummary = (user) => workspaceDefs.map(([key, label]) => `<label class="pill"><input type="checkbox" name="workspaceAccess" value="${escapeHtml(key)}" ${userWorkspaces(user).includes(key) ? 'checked' : ''} disabled> ${escapeHtml(label)}</label>`).join('');

      const editor = (user = { roles: ['client'], isActive: true }) => {
        root.innerHTML = `<section class="module-page stack"><form class="card module-section stack" data-user-editor><div class="module-header"><div><p class="eyebrow">Company Management</p><h2 class="module-title">${user.id ? 'Edit' : 'Create'} User</h2><p class="module-description">Profile fields, active status, role/rank, worker/client classification, and workspace access persist after refresh.</p></div></div><div class="form-grid"><label class="field"><span>Name</span><input name="fullName" value="${escapeHtml(user.fullName || user.full_name || '')}" placeholder="Full name"></label><label class="field"><span>Email</span><input name="email" value="${escapeHtml(user.email || '')}" placeholder="name@example.com" ${user.id ? 'readonly' : ''}></label><label class="field"><span>Phone</span><input name="phone" value="${escapeHtml(user.phone || '')}" placeholder="Phone number"></label><label class="field"><span>Active status</span><select name="isActive"><option value="true" ${user.isActive !== false ? 'selected' : ''}>Active</option><option value="false" ${user.isActive === false ? 'selected' : ''}>Inactive</option></select></label><label class="field"><span>Company</span><input name="companyName" value="${escapeHtml(user.companyName || user.company_name || '')}" placeholder="Company"></label><label class="field"><span>Worker / Client classification</span><select id="classification"><option value="client" ${asArray(user.roles).includes('client') ? 'selected' : ''}>Client</option><option value="worker" ${asArray(user.roles).includes('worker') ? 'selected' : ''}>Worker</option><option value="staff" ${!asArray(user.roles).some((role) => ['client','worker'].includes(role)) ? 'selected' : ''}>Staff / Admin</option></select></label></div><section class="module-card card stack"><h3>Role / Rank</h3><div class="module-grid">${roleCheckboxes(user)}</div></section><section class="module-card card stack"><h3>Workspace Access</h3><p>Workspace access is calculated from selected roles and the role permission matrix. Edit roles in Roles / Ranks to change workspace permission assignments.</p><div class="module-grid" id="workspace-summary">${workspaceSummary(user)}</div></section><p class="notice" id="user-save-status">Ready to save.</p><div class="action-row"><button class="btn" type="submit" id="save-user">Save User</button>${user.email ? `<button class="btn secondary" type="button" id="send-user-magic">Send Magic Link</button><button class="btn secondary" type="button" id="reset-user-magic">Reset Magic Link</button>` : ''}<button class="btn secondary" type="button" id="cancel-user">Cancel</button></div></form></section>`;
      ...
      const reactivate = async (id, button) => {
        ...
        try {
          ...
          await api.patch('/api/admin/users', { userId: id, fullName: user?.fullName || '', phone: user?.phone || '', companyName: user?.companyName || '', roles: asArray(user?.roles).length ? asArray(user.roles) : ['client'], currentRoles: asArray(user?.roles), isActive: true });
