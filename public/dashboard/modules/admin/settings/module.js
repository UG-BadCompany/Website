(() => {
  ...
  async function mountAdminSettings(context = {}) { const { api, router } = context; const mountRoot = window.TAModuleRoot.resolve(context); if (!mountRoot?.querySelector) throw new TypeError('Module root element was not found.'); const root = mountRoot;
    const tabs = {
    ...
    const render = () => {
      ...
      mountRoot.querySelector('[data-admin-settings]').addEventListener('submit', async (event) => {
        ...
        mountRoot.querySelector('[data-settings-status]').textContent = 'Saving through company settings backend...';
        try {
          ...
          mountRoot.querySelector('[data-settings-status]').textContent = error.message || 'Settings save failed.';
    ...
    mountRoot.innerHTML = '<article class="card module-loading"><h3>Loading Admin Settings</h3><p>Preparing allowed business settings.</p></article>';
    ...
    catch (error) { mountRoot.innerHTML = `<article class="card module-error"><h3>Settings loaded with limited data</h3><p>${escapeHtml(error.message || 'Company settings unavailable.')}</p></article>`; }
    render();
  }
  ...
  async function mountSystemCenter(context = {}) { const { api } = context; const mountRoot = window.TAModuleRoot.resolve(context); if (!mountRoot?.querySelector) throw new TypeError('Module root element was not found.');
    let active = 'overview';
    mountRoot.innerHTML = '<article class="card module-loading"><h3>Loading System Center</h3><p>Checking install, module, platform, installer, and health status.</p></article>';
    ...
    const render = () => {
      mountRoot.innerHTML = `<section class="module-page stack system-center-page"><div class="card module-header"><div><p class="eyebrow">Owner Workspace</p><h2 class="module-title">📊 System Center</h2><p class="module-description">One consolidated place for system overview, modules, platform defaults, installer state, and health checks. No duplicate settings rows or repeated fake actions.</p></div></div><div class="module-stat-grid">${statusCard('🗄️', 'Database', data.database?.ok ? 'Ready' : 'Limited', 'Database').replace('system-health-card module-card', 'module-stat stat-card')}${statusCard('λ', 'Functions', errors.length ? 'Warnings' : 'Checked', 'Functions').replace('system-health-card module-card', 'module-stat stat-card')}${statusCard('🔐', 'Auth', data.auth?.ok ? 'Ready' : 'Active', 'Auth').replace('system-health-card module-card', 'module-stat stat-card')}${statusCard('🤖', 'AI', data.ai?.ok ? 'Ready' : 'Safe', 'AI').replace('system-health-card module-card', 'module-stat stat-card')}${statusCard('🔒', 'Installer', data.installer_locked ? 'Locked' : 'Review', 'Installer').replace('system-health-card module-card', 'module-stat stat-card')}${statusCard('📦', 'Modules', modules.length || 5, 'Modules').replace('system-health-card module-card', 'module-stat stat-card')}</div><div class="card module-section stack">${tabButtons(active)}<div data-system-panel>${renderPanel()}</div></div></section>`;
      mountRoot.querySelectorAll('[data-system-tab]').forEach((button) => button.addEventListener('click', () => { active = button.dataset.systemTab; render(); }));
    };
    render();
  }

  async function mountAuditLogs(context = {}) { const { api } = context; const mountRoot = window.TAModuleRoot.resolve(context); if (!mountRoot?.querySelector) throw new TypeError('Module root element was not found.');
    mountRoot.innerHTML = '<article class="card module-loading"><h3>Loading Audit Logs</h3><p>Checking recent user, quote, permission, company, login, and admin activity.</p></article>';
    const { data, errors } = await fetchAll(api);
