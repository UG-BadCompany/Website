  ...
  window.TAModules.register({
    ...
    async mount({ root, api, user }) {
      ...
      const researchPanel = (record) => {
        ...
        return `<section class="module-card quote-section"><h3>Research Sources</h3><p><strong>Mode:</strong> ${escapeHtml(meta.research_mode || 'openai_first')} · <strong>OpenAI live search:</strong> ${meta.openai_live_search_used ? 'Used' : 'Not available'} · <strong>Fallback:</strong> ${meta.fallback_search_used ? 'Used' : 'Not used'}</p><p>${escapeHtml(meta.pricing_confidence_reason || 'Internal catalog allowances are available for admin review.')}</p>${sources.length ? `<ul class="quote-clean-list">${sources.map((source) => `<li>${escapeHtml(source.title || source.source || 'Pricing source')} ${source.price ? `· ${escapeHtml(source.price)}` : ''}</li>`).join('')}</ul>` : ''}</section>`;
      };
