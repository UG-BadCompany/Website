    ...
    async mount({ root, api, user }) {
      ...
      const saveDraft = async (send = false) => {
        ...
        if (send) {
          ...
          const badContent = [payload.summary, payload.title, ...allLines.map((line) => line.description || line.name || '')].some((value) => String(value || '').includes('quote_in_progress'));
          ...
          if (badContent) throw new Error('Remove quote_in_progress status text from visible quote content before sending.');
        }
