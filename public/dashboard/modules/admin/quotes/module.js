  ...
  window.TAModules.register({
    ...
    async mount({ root, api, user }) {
      ...
      const card = (record) => {
        ...
        return `<article class="module-record-card quote-accordion-card ${open?'active':''}" data-select="${escapeHtml(record.id)}" tabindex="0" aria-expanded="${open}"><div class="quote-card-main"><div class="quote-card-topline"><h3>${escapeHtml(record.customerName)}</h3><strong class="quote-card-price">${price ? money(price) : 'No price yet'}</strong></div><p class="quote-card-service">${escapeHtml(record.serviceType)}</p><p class="quote-card-address">${escapeHtml(record.address || 'No property listed')}</p><div class="quote-card-facts"><span class="status-badge">${escapeHtml(titleize(record.status))}</span><span class="status-badge ${badgeClass(record)}">${escapeHtml(c.label)}</span><span class="quote-confidence-pct">${c.pct}% confidence</span><span class="quote-card-updated">Updated ${escapeHtml(dateText(record.updatedAt))}</span></div><button class="btn tiny secondary quote-expand-btn" type="button">${open?'Collapse':'Expand'}</button></div>${open ? `<div class="quote-card-extra"><div class="quote-side-summary"><span>Short scope</span>${collapsibleText(record.description, 150)}</div><div class="quote-side-summary"><span>Recommended action</span><p>${escapeHtml(nextAction(record))}</p></div><div class="quote-card-actions"><button class="btn tiny" type="button" data-quote-side-action="open">Open Full Quote</button><button class="btn tiny secondary" type="button" data-quote-side-action="preview">Preview Client Quote</button><button class="btn tiny secondary" type="button" data-quote-side-action="pin">Pin</button><button class="btn tiny secondary" type="button" data-quote-side-action="collapse">Collapse</button></div></div>` : ''}</article>`;
      };
