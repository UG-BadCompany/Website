  ...
  window.TAModules.register({
    ...
    async mount({ root, api, user }) {
      ...
      const confidenceReasons = (record = {}) => {
        ...
        const positives = reasons.filter((reason) => !/missing|timed out|timeout|no photos|backfilled|unavailable|zero/i.test(String(reason)));
        const negatives = reasons.filter((reason) => /missing|timed out|timeout|no photos|backfilled|unavailable|zero/i.test(String(reason)));
      ...
      const lineBadge = (line = {}) => (line.backfilled || /backfill|compatibility pricing/i.test(`${line.source || ''} ${line.description || ''}`)) ? '<span class="status-badge warning quote-line-badge">Backfilled</span>' : '';
      const lineCards = (items, type) => asArray(items).map((line, index) => `<article class="quote-line-card" data-line-row="${type}" data-index="${index}"><div><h4>${escapeHtml(line.name || line.description || `${titleize(type)} item`)}</h4>${lineBadge(line)}<p>${escapeHtml(line.description || line.notes || '')}</p>${lineBadge(line) ? '<small>Created from compatibility pricing. Review before sending.</small>' : ''}</div><div class="quote-line-facts"><span>${type==='labor'?'Hours':'Qty'}<strong>${escapeHtml(line.quantity || line.hours || 1)} ${escapeHtml(line.unit || '')}</strong></span><span>${type==='labor'?'Rate':'Unit'}<strong>${money(line.unitCostCents || line.rateCents || 0)}</strong></span><span>Total<strong>${money(line.totalCents)}</strong></span><span>Confidence/source<strong>${escapeHtml(line.confidence || line.source || 'review')}</strong></span></div></article>`).join('') || `<article class="module-empty">No ${type} line items yet.</article>`;
      ...
      const card = (record) => {
        ...
        return `<article class="module-record-card quote-accordion-card ${open?'active':''} ${pinned?'pinned':''}" data-select="${escapeHtml(record.id)}" tabindex="0" aria-expanded="${open}"><div class="quote-card-summary"><span class="quote-chevron">${open?'▾':'▸'}</span><div><h3>${escapeHtml(record.title || record.serviceType)}</h3><p>${escapeHtml(record.kind === 'request' ? 'Request' : 'Quote')} · ${escapeHtml(titleize(record.status))}</p></div><span class="status-badge ${badgeClass(record)}">Confidence ${c.pct}%</span>${price ? `<strong>${money(price)}</strong>` : '<span>No price yet</span>'}</div><div class="quote-card-extra"><p><strong>${escapeHtml(record.customerName)}</strong> · ${escapeHtml(record.address)}</p><p>${escapeHtml(record.description).slice(0, 180)}</p><div class="quote-card-meta"><span>${escapeHtml(dateText(record.createdAt))}</span><span>Updated ${escapeHtml(dateText(record.updatedAt))}</span><span class="status-badge">${escapeHtml(nextAction(record))}</span><button class="btn tiny secondary" type="button" data-pin-quote="${escapeHtml(record.id)}">${pinned?'Unpin':'Pin open'}</button></div></div></article>`;
      };
