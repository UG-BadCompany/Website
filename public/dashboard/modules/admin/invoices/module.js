(() => {
  ...
  window.TAModules.register({ id:'admin.invoices', role:'admin', title:'Invoices', icon:'🧾', permissions:['invoices.manage'], async mount(context = {}) {
    const { api } = context;
    ...
    let data = { invoices: [], summary: {} }; let filter = 'active';
    ...
    const createInvoice = async () => {
      ...
      window.TAWorkflow?.emit?.('invoice:created', { invoiceId: result.invoice?.id }); toast(result.message || 'Invoice created.'); await load(); render();
    };
    const markPaid = async (invoice) => {
      ...
      window.TAWorkflow?.emit?.('payment:verified', { invoiceId: invoice.id }); toast(result.message || 'Invoice marked paid.'); await load(); render();
    };
    ...
    const isActiveInvoice = (invoice = {}) => ['draft', 'open', 'sent', 'payment_pending', 'overdue'].includes(String(invoice.status || '').toLowerCase());
    const render = () => {
      const invoices = data.invoices || [];
      mountRoot.innerHTML = `<section class="module-page stack"><div class="module-hero module-header card"><div><p class="eyebrow">Admin / Manager workflow</p><h2 class="module-title">🧾 Invoices</h2><p class="module-description">Handle invoice-ready work, sent invoices, payment pending, paid, and void/cancelled invoices after Work Orders are complete.</p></div><div class="action-row"><button class="btn" type="button" data-create>Create Invoice</button><button class="btn secondary" type="button" data-refresh>Refresh</button></div></div><div class="module-tabs">${['active','all','paid','void'].map((item) => `<button class="btn secondary ${filter === item ? 'active' : ''}" type="button" data-filter="${item}">${esc(title(item))}</button>`).join('')}</div><div class="module-stat-grid"><article class="module-stat stat-card"><span>🧾</span><strong>${esc(data.summary?.open ?? 0)}</strong><small>Open</small></article><article class="module-stat stat-card"><span>✅</span><strong>${esc(data.summary?.paid ?? 0)}</strong><small>Paid</small></article><article class="module-stat stat-card"><span>💳</span><strong>${esc(money(data.summary?.amountDueCents || 0))}</strong><small>Due</small></article><article class="module-stat stat-card"><span>📥</span><strong>${esc(money(data.summary?.amountCollectedCents || 0))}</strong><small>Collected</small></article></div><div class="module-panel module-section card"><h3>Invoice queue</h3><div class="module-record-list">${invoices.length ? invoices.map((invoice) => `<article class="module-record-card"><div><p class="eyebrow">${esc(title(invoice.status))}</p><h3>${esc(invoice.title || 'Invoice')}</h3><p>${esc(invoice.client?.fullName || invoice.client?.email || invoice.jobRequest?.serviceType || 'Customer invoice')}</p><small>${esc(money(invoice.amountCents))} · Due ${esc(date(invoice.dueAt))} · ${esc(invoice.provider?.name || 'manual')}</small></div><div class="module-record-actions"><button class="btn secondary" type="button" data-view="${esc(invoice.id)}">View Invoice</button>${isActiveInvoice(invoice) ? `<button class="btn secondary" type="button" data-link="${esc(invoice.id)}">Payment Link</button><button class="btn secondary" type="button" data-paid="${esc(invoice.id)}">Mark Paid</button><button class="btn secondary" type="button" data-void="${esc(invoice.id)}">Void Invoice</button>` : ''}</div></article>`).join('') : '<article class="module-empty"><h3>No invoices</h3><p>Invoice-ready work orders and created invoices will appear here.</p></article>'}</div></div></section>`;
      mountRoot.querySelector('[data-create]')?.addEventListener('click', createInvoice);
      mountRoot.querySelector('[data-refresh]')?.addEventListener('click', async () => { await load(); render(); });
      mountRoot.querySelectorAll('[data-filter]').forEach((btn) => btn.addEventListener('click', async () => { filter = btn.dataset.filter; await load(); render(); }));
      mountRoot.querySelectorAll('[data-view]').forEach((btn) => btn.addEventListener('click', () => TAModuleKit.openDetail(root, { title:'Invoice', detailMode:'readonly', readOnlyDetail:true, detailSections:['Customer information','Invoice lines','Payment status','Due date','Payment link','Notes'], canEdit:false }, invoices.find((i) => String(i.id) === btn.dataset.view))));
      mountRoot.querySelectorAll('[data-link]').forEach((btn) => btn.addEventListener('click', () => paymentLink(invoices.find((i) => String(i.id) === btn.dataset.link))));
      mountRoot.querySelectorAll('[data-paid]').forEach((btn) => btn.addEventListener('click', () => markPaid(invoices.find((i) => String(i.id) === btn.dataset.paid))));
      mountRoot.querySelectorAll('[data-void]').forEach((btn) => btn.addEventListener('click', () => voidInvoice(invoices.find((i) => String(i.id) === btn.dataset.void))));
    };
    mountRoot.innerHTML = '<section class="stack"><article class="card"><h2>Invoices</h2><p>Loading invoices...</p></article></section>';
    await load(); render();
