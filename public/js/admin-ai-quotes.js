...
// Backward-compatible admin review script.
// User-facing/admin label is now "Estimate Drafts", not "AI Quotes".
...
(() => {
  ...
  const localBackups = () => {
    try {
      const newer = JSON.parse(localStorage.getItem('estimateDraftBackups') || '[]').map((x, i) => ({
        id: `local_est_${i}`,
        createdAt: x.savedAt,
        estimateDraft: x.draft,
      }));

      const old = JSON.parse(localStorage.getItem('aiQuoteDraftBackups') || '[]').map((x, i) => ({
        id: `local_ai_${i}`,
        createdAt: x.savedAt,
        estimateDraft: x.aiDraft,
      }));
  ...
  const laborTable = (items = []) => {
    ...
    return `<div class="table-wrap"><table><thead><tr><th>Labor</th><th>Hours</th><th>Notes</th></tr></thead><tbody>${items.map((l) => `
      <tr><td>${esc(l.name)}<br><small>${esc(l.description || '')}</small></td><td>${esc(l.low_hours)}–${esc(l.high_hours)}</td><td>${esc(l.notes || '')}</td></tr>
    `).join('')}</tbody></table></div>`;
  };

  const materialsTable = (items = []) => {
    if (!items.length) return '<p>No materials listed.</p>';
    return `<div class="table-wrap"><table><thead><tr><th>Material</th><th>Qty</th><th>Range</th></tr></thead><tbody>${items.map((m) => `
      <tr><td>${esc(m.name)}<br><small>${esc(m.notes || '')}</small></td><td>${esc(m.quantity)}</td><td>${money(m.estimated_cost_low)}–${money(m.estimated_cost_high)}</td></tr>
    `).join('')}</tbody></table></div>`;
  };

  const renderDraft = (entry) => {
    const d = entry.estimateDraft || entry.aiDraft || entry;
    const req = d.request || entry.requestPayload || {};
    const totals = d.totals || {};

    return `
      <article class="draft-card">
        <h2>${esc(d.job_summary || 'Estimate draft')}</h2>
        <div>
          <span class="pill ${d.quote_ready ? 'good' : 'warn'}">${d.quote_ready ? 'Ready for review' : 'Needs info'}</span>
        ...
        <p><strong>Total:</strong> ${money(totals.total_low)}–${money(totals.total_high)} | <strong>Labor:</strong> ${esc(totals.labor_hours_low || 0)}–${esc(totals.labor_hours_high || 0)} hrs</p>
        ...
        <div class="card" style="margin-top:16px"><h3>Customer Estimate Draft</h3><pre>${esc(d.customer_facing_quote || '')}</pre></div>
  ...
  const load = async () => {
    setStatus('Loading estimate drafts…');
    root.innerHTML = '';
    ...
    if (!drafts.length) {
      setStatus('No estimate drafts found yet.');
      root.innerHTML = '<div class="draft-card"><h2>No estimate drafts yet</h2><p>Submit a Request Estimate form to create the first draft.</p></div>';
      return;
    }

    setStatus(`${drafts.length} estimate draft(s) found.`);
