...
If Netlify logs still show deleted stale migrations such as `0004_custom_roles_permissions.sql`, duplicate `0009_*` files, `0010_invoices_payments.sql`, or `0012_quote_payment_completion_controls.sql`, the prebuild migration check removes those cached files even if their replacement migration is absent from the stale checkout. If Netlify Database still reports duplicates before the prebuild can run, trigger **Clear cache and deploy site** in Netlify so old migration files are removed from the cached checkout before Netlify Database validates migrations.
