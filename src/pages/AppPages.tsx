...
function StatusPipeline({ statuses }: { statuses?: string[] }) { if (!statuses?.length) return null; return <div className="status-pipeline">{statuses.map((status) => <span key={status}>{status}</span>)}</div>; }
...
function ModuleList({ rows, columns, onSelect }: { rows: any[]; columns: ModuleColumn[]; onSelect: (row: any) => void }) { return <div className="module-list">{rows.map((row) => <button className="module-row" key={row.id || JSON.stringify(row)} onClick={() => onSelect(row)}>{columns.map((col) => <span key={col.key} data-label={col.label}>{col.kind === 'status' ? <StatusBadge status={displayValue(row,col)} /> : col.kind === 'badge' ? <Badge>{displayValue(row,col)}</Badge> : displayValue(row,col)}</span>)}</button>)}</div>; }
...
function ModuleCreateForm({ config, onDone }: { config: ModuleConfig; onDone: () => void }) {
  ...
  return <form className="card form module-create" onSubmit={submit}><h3>{config.actionLabel || 'Create record'}</h3>{config.createFields.map((field) => <label key={field}><span className="field-label">{field}</span><input value={values[field] || ''} onChange={(e) => setValues({ ...values, [field]: e.target.value })}/></label>)}<Button>Save</Button>{status && <p className="muted">{status}</p>}</form>;
}
function ModulePage({ config }: { config: ModuleConfig }) {
  ...
  return <Protected permission={config.permission}><AppLayout title={config.title}><div className="module-shell"><PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description}/><ModuleToolbar query={query} setQuery={setQuery} canManage={auth.can(config.managePermission)} actionLabel={config.actionLabel} onCreate={() => setShowCreate(!showCreate)}/><StatusPipeline statuses={config.pipeline || state.data?.statuses}/>{state.loading && <LoadingState title={`Loading ${config.title}`} lines={3}/>} {state.error && <div className="card error-panel"><h2>Could not load {config.title}</h2><p>{state.error}</p><Button variant="secondary" onClick={state.reload}>Try again</Button></div>} {!state.loading && !state.error && rows.length === 0 && <EmptyState title={config.emptyTitle} description={config.emptyDescription} action={auth.can(config.managePermission) ? <Button onClick={() => setShowCreate(true)}>Create first record</Button> : undefined}/>} {showCreate && auth.can(config.managePermission) && <ModuleCreateForm config={config} onDone={() => { state.reload(); setShowCreate(false); }}/>} {rows.length > 0 && <ModuleList rows={rows} columns={config.columns} onSelect={setSelected}/>} {selected && <DetailDrawer row={selected} config={config} onClose={() => setSelected(null)}/>}</div></AppLayout></Protected>;
}
