...
import { Home, LayoutDashboard, MessageSquare, Settings, UserRound, Wrench } from 'lucide-react';
...
export function PublicLayout({ children }: { children: ReactNode }) {
  return <>
    <header className="site-header"><Link href="/" className="brand"><span>⌂</span>ContractorOS</Link><nav><Link href="/about">About</Link><Link href="/services">Services</Link><Link href="/contact">Contact</Link><Link href="/request-estimate" className="button small">Request Estimate</Link><Link href="/login">Login</Link></nav></header>
    <main>{children}</main>
    <footer className="footer">ContractorOS Foundation • Netlify-first • PostgreSQL • Resend • Square-ready</footer>
  </>;
}
...
export function AppLayout({ title, children }: { title: string; children: ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><Link href="/" className="brand"><span>⌂</span>ContractorOS</Link>{nav.map((item) => <NavLink key={item.href} href={item.href}><item.icon size={18}/>{item.label}</NavLink>)}</aside>
