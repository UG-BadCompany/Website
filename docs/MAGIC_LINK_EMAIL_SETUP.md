...
When `RESEND_API_KEY` or `MAGIC_LINK_FROM_EMAIL` is missing/still a placeholder, or Resend rejects the sender, the API only returns `devMagicLink` when development links are explicitly enabled with `ALLOW_DEV_MAGIC_LINKS=true` or when `NODE_ENV` is `development`/`test`. Leave `ALLOW_DEV_MAGIC_LINKS=false` or unset in production.

Do not share development magic links with customers. They are intended only for setup/testing, and production API responses should not expose them.
...
5. Leave `ALLOW_DEV_MAGIC_LINKS=false` or unset in production.
6. Redeploy the site.
7. Test `/login/` with your own email and confirm an email arrives.
