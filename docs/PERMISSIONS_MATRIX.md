...
| `GET /api/client/job-requests` | Yes | `client` or `admin` | Returns job requests scoped to the signed-in client account. |
...
- A user with only `client` sees only client sections, including the requests attached to their account; worker and admin sections stay hidden in the browser and remain blocked by server-side API checks.
