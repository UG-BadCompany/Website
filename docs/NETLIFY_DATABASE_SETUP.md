...
- The function accepts JSON with `name`, `phone`, `email`, `city`, `streetAddress`, `service`, `timeframe`, `description`, and the Netlify honeypot field `bot-field`.
- `name`, `phone`, `email`, `city`, `streetAddress`, `service`, and `description` are required.
...
Netlify Database tracks applied migration files by name and contents. Do not edit already-applied migration files in place; add a later migration for any new schema change. The `0004_work_order_schedule.sql` file is intentionally checksum-locked by `scripts/check-netlify-migrations.mjs` because Netlify has already applied it.

`0006_job_request_schedule_dates.sql` intentionally repeats the idempotent schedule-column statements from `0004_work_order_schedule.sql` for fresh checkouts and stale deploy-cache recovery. Both files use `if not exists`; keep them byte-for-byte aligned unless Netlify's applied migration ledger is updated with `netlify db migrations pull`.
