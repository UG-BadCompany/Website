...
1. User opens `/login/`.
2. Existing active users request a magic link; new clients start by submitting Request Work so the backend can create their account first.
3. The magic-link endpoint returns the same generic response for unknown emails and does not create sign-in tokens for them.
4. A Netlify Function validates the session/token.
5. The function links the token to the matching `app_users` row; request-created client links may create/update the client profile when the request is submitted.
...
- Email delivery uses Resend when `RESEND_API_KEY` and a from-address are configured. Without Resend, the endpoint returns a development-only magic link only when `ALLOW_DEV_MAGIC_LINKS=true` or `NODE_ENV` is `development`/`test`; production responses must not expose raw sign-in links.
- `/api/auth/magic-link` only issues sign-in tokens for active existing users. Unknown emails receive the same generic response without a token so portal access remains account-first.
