...
import { addAudit, calculateQuote, createQuote, moneyFromForm, nowIso, updateJobStatus } from "../../lib/database";
import { sendEmail } from "../../lib/email";
import { renderQuotePdf } from "../../lib/pdf";
import { uploadGeneratedDocument } from "../../lib/storage";
...
export async function POST(request: Request) {
  ...
  const labor: QuoteLineItem = { id: makeId(), type: "Labor", description: form.get("laborDescription")?.toString() || "Labor", quantity: moneyFromForm(form.get("laborQuantity")) || 1, unitPrice: moneyFromForm(form.get("laborRate")) };
  const materials: QuoteLineItem = { id: makeId(), type: "Materials", description: form.get("materialsDescription")?.toString() || "Materials", quantity: 1, unitPrice: moneyFromForm(form.get("materialsAmount")) };
  ...
  if (jobRequestId !== "manual") await updateJobStatus(jobRequestId, "Quote sent");
  const pdfBuffer = await renderQuotePdf(quote);
  const doc = await uploadGeneratedDocument(`${quote.quoteNumber}.pdf`, pdfBuffer);
  await sendEmail({ to: quote.clientEmail, event: "quote_sent", subject: `T&A Contracting quote ${quote.quoteNumber}`, body: `Your quote total is ${quote.total}. Review it at /portal/quotes/${quote.id}. PDF: ${doc.url}`, attachments: [{ filename: `${quote.quoteNumber}.pdf`, content: pdfBuffer }] });
  await addAudit({ actor: session.id, action: "send_quote", entityType: "Quote", entityId: quote.id, details: `Quote ${quote.quoteNumber} sent` });
