(() => {
  ...
  const defaults = {
    ...
    whyChooseTitle: 'Why choose T&A Contracting',
    whyChooseCards: [
    ...
    footerText: 'Customer-focused repair, maintenance, installation, punch-list, and property improvement help in Arizona.', footerPhone: '', footerEmail: '', footerAddress: 'Phoenix, Goodyear, Surprise, Scottsdale, Chandler, and surrounding Arizona communities.', socialLinks: {}, licenseText: '',
  ...
  window.TAHomepageDefaults = { SERVICE_CATEGORIES, defaults };
  const merge = (settings = {}) => ({ ...defaults, ...settings, sectionVisibility: { ...defaults.sectionVisibility, ...(settings.sectionVisibility || {}) }, servicesConfig: Array.isArray(settings.servicesConfig) ? settings.servicesConfig : defaults.servicesConfig, whyChooseCards: Array.isArray(settings.whyChooseCards) ? settings.whyChooseCards : defaults.whyChooseCards, citiesServed: Array.isArray(settings.citiesServed) ? settings.citiesServed : defaults.citiesServed });
  ...
  const serviceCards = (settings) => sorted(settings.servicesConfig).filter((item) => item.enabled !== false && SERVICE_CATEGORIES.includes(item.name)).map((item) => `<article class="card service-card"><span>${esc(item.name)}</span><h3>${esc(item.name)}</h3><p>${esc(item.description || `${item.name} support for Arizona properties.`)}</p></article>`).join('');
  ...
  const renderDynamic = (settings, gallery) => {
    ...
    if (footer) footer.innerHTML = visible.contactFooter === false ? '' : `<div class="container footer-premium footer-grid-customer"><div><h2><span data-company-name>T&amp;A Contracting</span></h2><p>${esc(settings.footerText)}</p>${settings.licenseText ? `<small>${esc(settings.licenseText)}</small>` : ''}</div><div><h3>Contact</h3>${settings.footerPhone ? `<a href="tel:${esc(settings.footerPhone)}">${esc(settings.footerPhone)}</a>` : ''}${settings.footerEmail ? `<a href="mailto:${esc(settings.footerEmail)}">${esc(settings.footerEmail)}</a>` : ''}${Object.entries(settings.socialLinks || {}).map(([label,url]) => `<a href="${esc(url)}" target="_blank" rel="noopener">${esc(label)}</a>`).join('')}<a href="#estimate">Request Estimate</a></div><div><h3>Service Area</h3><p>${esc(settings.footerAddress || settings.citiesServed.join(', '))}</p></div></div>`;
    window.TACompany?.apply?.(window.TACompany.current || window.TACompany.fallback || {});
    document.dispatchEvent(new CustomEvent('ta:homepage-rendered'));
  };
  document.addEventListener('DOMContentLoaded', async () => {
    let settings = defaults, gallery = [];
    try { const data = await window.TAApi.get('/.netlify/functions/homepage-settings'); settings = merge(data.settings || data.homepage || data); } catch { settings = merge({}); }
    try { const data = await window.TAApi.get('/.netlify/functions/homepage-gallery'); gallery = data.gallery || []; } catch { gallery = []; }
    renderDynamic(settings, gallery);
  });
})();
