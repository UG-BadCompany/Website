(() => {
  ...
  const buildDefaults = (company = window.TACompany?.current || {}) => {
    ...
    return {
    heroBadge: 'Trusted local property services', heroLogoSize: 'large',
  ...
  const logoMarkup = (company = {}, settings = {}) => {
    ...
    const initials = name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'YC';
    const logo = company.logoUrl || company.logo_url || company.brandLogoUrl || company.brand_logo_url || '';
    const size = ['compact', 'large', 'xl'].includes(settings.heroLogoSize) ? settings.heroLogoSize : 'large';
    return `<div class="hero-brand-lockup hero-logo-${esc(size)}"><div class="hero-logo-mark">${logo ? `<img src="${esc(logo)}" alt="${esc(name)} logo" loading="eager" onerror="this.remove();this.parentElement.textContent='${esc(initials)}'">` : esc(initials)}</div><div><span>${esc(settings.heroBadge || 'Trusted local property services')}</span><strong>${esc(name)}</strong></div></div>`;
  };
