(() => {
  ...
  const isPaid = (invoice = {}) => ['paid', 'payment_verified'].includes(String(invoice.status || '').toLowerCase());
  const serviceSummary = (invoice = {}) => invoice.jobRequest?.serviceType || invoice.title || 'Service invoice';
  const printableInvoice = (invoice = {}) => `<!doctype html><html><head><title>${esc(invoice.title || 'Invoice')}</title><style>:root{color-scheme:light dark}body{font-family:system-ui,sans-serif;margin:2rem;color:CanvasText;background:Canvas}.invoice{max-width:760px;margin:auto}.row{display:flex;justify-content:space-between;border-bottom:1px solid color-mix(in srgb,CanvasText 18%,transparent);padding:.75rem 0}.total{font-size:1.4rem;font-weight:800}.muted{color:color-mix(in srgb,CanvasText 62%,transparent)}button{border:1px solid color-mix(in srgb,CanvasText 18%,transparent);border-radius:12px;padding:.7rem 1rem;background:Canvas;color:CanvasText}@media print{button{display:none}}</style></head><body><main class="invoice"><button onclick="window.print()">Print / Save PDF</button><h1>${esc(invoice.title || 'Invoice')}</h1><p class="muted">Invoice #${esc(invoice.id || '')}</p><div class="row"><span>Service summary</span><strong>${esc(serviceSummary(invoice))}</strong></div><div class="row"><span>Due date</span><strong>${esc(date(invoice.dueAt))}</strong></div><div class="row"><span>Payment status</span><strong>${esc(status(invoice.status))}</strong></div><div class="row total"><span>Amount due</span><strong>${esc(money(isPaid(invoice) ? 0 : invoice.amountCents))}</strong></div><p>${esc(invoice.jobRequest?.streetAddress || '')} ${esc(invoice.jobRequest?.city || '')}</p></main><script>window.print()</script></body></html>`;
  const openInvoice = (invoice) => {
    const modal = document.getElementById('modal-root') || document.body;
    modal.innerHTML = `<div class="module-drawer"><article class="card module-readonly-detail stack"><div class="module-editor-head"><div><p class="eyebrow">Client invoice · view only</p><h2>${esc(invoice.title || 'Invoice')}</h2><p>No admin fields or internal notes are shown here.</p></div><button class="btn secondary" type="button" data-close-invoice>Close</button></div><div class="module-editor-grid module-readonly-grid"><section class="module-readonly-field"><span>Invoice number</span><p>${esc(invoice.id)}</p></section><section class="module-readonly-field"><span>Service summary</span><p>${esc(serviceSummary(invoice))}</p></section><section class="module-readonly-field"><span>Amount due</span><p>${esc(money(isPaid(invoice) ? 0 : invoice.amountCents))}</p></section><section class="module-readonly-field"><span>Due date</span><p>${esc(date(invoice.dueAt))}</p></section><section class="module-readonly-field"><span>Payment status</span><p>${esc(status(invoice.status))}</p></section></div></article></div>`;
  ...
  window.TAModules.register({ id:'client.invoices', role:'client', title:'My Invoices', icon:'🧾', permissions:[], async mount(context = {}) {
    ...
    const pay = async (invoice, button) => {
      if (isPaid(invoice)) { window.TAUi?.toast?.('This invoice is already paid.', 'info'); return; }
    ...
    mountRoot.innerHTML = '<section class="stack"><article class="card module-loading"><h2>My Invoices</h2><p>Preparing invoices.</p></article></section>';
    await load(); render();
