  ...
  window.TAModules.register({
    ...
    async mount({ root, api, user }) {
      ...
      const card = (record) => {
        ...
        return `<article class="module-record-card quote-accordion-card ${open?'active':''}" data-select="${escapeHtml(record.id)}" tabindex="0" aria-expanded="${open}"><div class="quote-card-main"><h3>${escapeHtml(record.customerName)}</h3><p class="quote-card-service">${escapeHtml(record.serviceType)}</p><p class="quote-card-address">${escapeHtml(addressParts[0] || record.address || 'No property listed')}</p>${addressParts[1] ? `<p class="quote-card-address">${escapeHtml(addressParts[1])}</p>` : ''}<div class="quote-card-badges"><span class="status-badge">${escapeHtml(titleize(record.status))}</span><span class="status-badge ${badgeClass(record)}">${escapeHtml(c.label)}</span><span>${c.pct}%</span></div><strong class="quote-card-price">${price ? money(price) : 'No price yet'}</strong><p class="quote-card-updated">Updated: ${escapeHtml(dateText(record.updatedAt))}</p><button class="btn tiny secondary quote-expand-btn" type="button">${open?'Collapse':'Expand'}</button></div>${open ? `<div class="quote-card-extra">${collapsibleText(record.description, 160)}<div class="quote-card-meta"><span>${escapeHtml(record.kind === 'request' ? 'Request' : 'Quote')} · Created ${escapeHtml(dateText(record.createdAt))}</span><span>${escapeHtml(nextAction(record))}</span></div></div>` : ''}</article>`;
      };
