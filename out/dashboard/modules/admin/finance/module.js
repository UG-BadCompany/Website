(() => {
  ...
  window.TAModules.register({ id:'admin.finance', role:'admin', title:'Finance', icon:'📊', permissions:[], async mount(context = {}) {
    const { api } = context;
    ...
    const render = () => {
      ...
      mountRoot.innerHTML = `<section class="module-page stack admin-finance-page"><div class="module-hero module-header card"><div><p class="eyebrow">Finance</p><h2 class="module-title">📊 Finance</h2><p class="module-description">Live revenue, invoice, quote, payment, aging, and Square payment status from real invoice/payment/quote/work-order records.</p></div><button class="btn secondary" data-refresh-finance>Refresh</button></div><div class="module-stat-grid">${[
      ...
      mountRoot.querySelector('[data-refresh-finance]')?.addEventListener('click', async () => { await load(); render(); });
    };
    mountRoot.innerHTML = '<section class="stack"><article class="card"><h2>Finance</h2><p>Loading real finance data...</p></article></section>';
    await load(); render();
