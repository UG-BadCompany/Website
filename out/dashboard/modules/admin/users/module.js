  ...
  window.TAModules.register({
    ...
    async mount(context = {}) { const { api } = context; const mountRoot = window.TAModuleRoot.resolve(context); if (!mountRoot?.querySelector) throw new TypeError('Module root element was not found.'); const root = mountRoot;
      ...
      const requireForms = () => {
        ...
        const status = mountRoot.querySelector('#user-save-status');
      ...
      const load = async () => {
        mountRoot.innerHTML = '<article class="card module-loading"><h3>Loading users</h3><p>Pulling company users, roles, ranks, and workspace access.</p></article>';
        try {
          ...
          mountRoot.innerHTML = `<section class="module-page stack"><article class="card module-error"><h2>Users</h2><p>${escapeHtml(error.message || 'Unable to load users')}</p><button class="btn secondary" type="button" id="retry-users">Retry</button></article></section>`;
          mountRoot.querySelector('#retry-users')?.addEventListener('click', load);
        }
      };
      ...
      const render = () => {
        const summary = stats();
        mountRoot.innerHTML = `<section class="module-page stack users-page">
        ...
        mountRoot.querySelector('#new-user').addEventListener('click', () => editor());
        mountRoot.querySelector('#user-search').addEventListener('input', renderList);
        mountRoot.querySelector('#role-filter').addEventListener('change', renderList);
        renderList();
      };

      const renderList = () => {
        const q = mountRoot.querySelector('#user-search')?.value?.toLowerCase() || '';
        const rf = mountRoot.querySelector('#role-filter')?.value || '';
        ...
        const list = mountRoot.querySelector('#user-list');
        ...
        mountRoot.querySelectorAll('[data-edit]').forEach((button) => button.addEventListener('click', () => editor(data.users.find((user) => String(user.id) === button.dataset.edit))));
        mountRoot.querySelectorAll('[data-delete]').forEach((button) => button.addEventListener('click', () => deactivate(button.dataset.delete, button)));
        mountRoot.querySelectorAll('[data-reactivate]').forEach((button) => button.addEventListener('click', () => reactivate(button.dataset.reactivate, button)));
        mountRoot.querySelectorAll('[data-magic],[data-reset-magic]').forEach((button) => button.addEventListener('click', () => sendMagic(button.dataset.magic || button.dataset.resetMagic, button)));
      };
      ...
      const editor = (user = { roles: ['client'], isActive: true }) => {
        ...
        const updateClassification = () => {
          const selected = mountRoot.querySelector('#classification')?.value;
          if (selected === 'worker') { const input = mountRoot.querySelector('input[name="roles"][value="worker"]:not(:disabled)'); if (input) input.checked = true; }
          if (selected === 'client') { const input = mountRoot.querySelector('input[name="roles"][value="client"]:not(:disabled)'); if (input) input.checked = true; }
        };
        mountRoot.querySelector('#classification')?.addEventListener('change', updateClassification);
        mountRoot.querySelector('#cancel-user').addEventListener('click', render);
        mountRoot.querySelector('#send-user-magic')?.addEventListener('click', (event) => sendMagic(user.email, event.currentTarget));
        mountRoot.querySelector('#reset-user-magic')?.addEventListener('click', (event) => sendMagic(user.email, event.currentTarget));
        mountRoot.querySelector('[data-user-editor]').addEventListener('submit', async (event) => {
          ...
          const saveButton = mountRoot.querySelector('#save-user');
          mountRoot.querySelector('#user-save-status').textContent = 'Saving user, role/rank, and workspace access...';
          ...
          try {
            ...
            mountRoot.querySelector('#user-save-status').textContent = error.message || 'User save failed.';
