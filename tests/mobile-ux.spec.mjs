...
test('mobile header links and dashboard module shortcuts remain functional', async () => {
  ...
  assert.match(dashboard, /mobile-more-menu[\s\S]*Dashboard[\s\S]*Inventory[\s\S]*Invoices \/ Finance[\s\S]*Customers[\s\S]*Employees \/ Workers[\s\S]*Admin[\s\S]*Reports[\s\S]*Settings[\s\S]*Logout/, 'More menu should expose remaining modules and logout.');
