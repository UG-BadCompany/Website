...
test('npm prebuild uses the CommonJS Netlify migration guard', async () => {
  const packageJson = JSON.parse(await readFile(new URL('../package.json', import.meta.url), 'utf8'));
  const prebuildScript = await readFile(new URL('../scripts/prebuild-netlify-migrations.cjs', import.meta.url), 'utf8');

  assert.equal(packageJson.scripts.prebuild, 'node scripts/prebuild-netlify-migrations.cjs');
  assert.match(prebuildScript, /restoreTrackedFileFromHead\('netlify\/functions\/me\.mjs'\)/);
  assert.match(prebuildScript, /process\.env\.NETLIFY !== 'true'/);
  await assert.doesNotReject(execFileAsync(process.execPath, ['--check', 'scripts/prebuild-netlify-migrations.cjs']));
});
...
test('npm prebuild uses the CommonJS Netlify migration guard', async () => {
  ...
  assert.equal(packageJson.scripts.prebuild, 'node scripts/prebuild-netlify-migrations.cjs');
  assert.match(prebuildScript, /restoreTrackedFileFromHead\('netlify\/functions\/me\.mjs'\)/);
  assert.match(prebuildScript, /const isNetlifyBuild = \(\) =>/);
  assert.match(prebuildScript, /process\.env\.BUILD_ID/);
  await assert.doesNotReject(execFileAsync(process.execPath, ['--check', 'scripts/prebuild-netlify-migrations.cjs']));
});
...
test('npm prebuild uses the CommonJS Netlify migration guard', async () => {
  ...
  assert.equal(packageJson.scripts.prebuild, 'node scripts/prebuild-netlify-migrations.cjs');
  assert.match(prebuildScript, /restoreTrackedFileFromHead\('netlify\/functions\/me\.mjs'\)/);
  assert.doesNotMatch(prebuildScript, /isNetlifyBuild/);
  await assert.doesNotReject(execFileAsync(process.execPath, ['--check', 'scripts/prebuild-netlify-migrations.cjs']));
});

test('Netlify function syntax checker restores tracked me function before checks', async () => {
  const checkerScript = await readFile(new URL('../scripts/check-netlify-functions.mjs', import.meta.url), 'utf8');

  assert.match(checkerScript, /restoreTrackedFileFromHead\('netlify\/functions\/me\.mjs'\)/);
  assert.doesNotMatch(checkerScript, /isNetlifyBuild/);
  await assert.doesNotReject(execFileAsync(process.execPath, ['--check', 'scripts/check-netlify-functions.mjs']));
  await assert.doesNotReject(execFileAsync(process.execPath, ['scripts/check-netlify-functions.mjs']));
});
...
test('Netlify function syntax checker repairs await helpers before checking cached me function', async () => {
  const { ensureAwaitHelpersAreAsync } = await import('../scripts/check-netlify-functions.mjs');
  const staleSource = `const loadCurrentUser = () => {
  const db = await getDatabase();
  return db;
};

function loadCurrentUserFallback() {
  const fallback = await loadCurrentUserFallback(db, sessionTokens);
  return fallback;
}
`;
  const repairedSource = ensureAwaitHelpersAreAsync(staleSource);

  assert.match(repairedSource, /const loadCurrentUser = async \(\) => \{/);
  assert.match(repairedSource, /async function loadCurrentUserFallback\(\) \{/);
});
