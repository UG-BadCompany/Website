...
test('dashboard user and role controls have their required handlers', async () => {
  ...
  assert.match(script, /const loadSignedInDashboard = async \(\) =>/, 'dashboard session check should use a named async loader');
  assert.match(script, /loadSignedInDashboard\(\);/, 'dashboard session loader should be invoked after declaration');
  assert.doesNotMatch(script, /fetch\('\/api\/me', \{ headers: \{ accept: 'application\/json' \} \}\)\n        \.then/, 'dashboard session check should not use the old inline promise chain');
