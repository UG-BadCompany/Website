...
test('magic-link endpoint only stores a hashed token for an existing active user', async () => {
  const original = {
    ALLOW_DEV_MAGIC_LINKS: process.env.ALLOW_DEV_MAGIC_LINKS,
    NODE_ENV: process.env.NODE_ENV,
  };
  process.env.ALLOW_DEV_MAGIC_LINKS = 'true';
  delete process.env.NODE_ENV;

  const db = createMockDb([[{ id: 'user-1' }]]);
  ...
  try {
    ...
    assert.match(db.queries[0].text, /from app_users/);
    assert.equal(db.queries[0].values[0], 'client@example.com');
    assert.match(db.queries[1].text, /insert into auth_magic_links/);
    assert.equal(db.queries[1].values[0], 'client@example.com');
    assert.equal(db.queries[1].values[1], hashToken('magic-token'));
  } finally {
    for (const [key, value] of Object.entries(original)) {
      if (value === undefined) {
        delete process.env[key];
...
test('magic-link endpoint does not create tokens for unknown emails', async () => {
  const db = createMockDb([[]]);
  const handler = createMagicLinkHandler({
    getDatabase: async () => db,
    makeToken: () => 'magic-token',
    sendEmail: async () => {
      throw new Error('Email should not be attempted for unknown users.');
    },
  });

  const response = await readJson(await handler(request({ email: 'unknown@example.com' }, 'POST', 'https://site.test/login/')));

  assert.equal(response.status, 200);
  assert.equal(response.body.ok, true);
  assert.equal(response.body.emailSent, false);
  assert.equal(response.body.devMagicLink, undefined);
  assert.match(response.body.message, /If this email can sign in/);
  assert.equal(db.queries.length, 1);
  assert.match(db.queries[0].text, /from app_users/);
});

test('magic-link endpoint hides development links unless explicitly enabled', async () => {
  const original = {
    ALLOW_DEV_MAGIC_LINKS: process.env.ALLOW_DEV_MAGIC_LINKS,
    NODE_ENV: process.env.NODE_ENV,
  };
  delete process.env.ALLOW_DEV_MAGIC_LINKS;
  ...
  try {
    assert.equal(shouldExposeDevMagicLinks(), false);
