...
const computeConfidence = ({ estimate = {}, payload = {}, research = {} }) => {
  ...
  const info = score01([has(text), has(payload.service || payload.service_category), has(payload.address || payload.streetAddress || payload.propertySummary), Boolean(payload.photosProvided || payload.files?.length || payload.photos?.length), /model|serial|btu|ton|amp|volt|brand/i.test(text), /\d+\s*(ft|feet|inch|in|ton|btu|amp|volt)/i.test(text), has(payload.timeframe || payload.preferredTimeframe), text.length > 40].filter(Boolean).length, 8);
  const knownJob = /mini\s*-?split|ductless|faucet|toilet|water heater|outlet|switch|drywall/i.test(text);
  const laborScore = score01([knownJob, labor.length > 0, pricedLabor.length === labor.length && labor.length > 0, text.length > 40, Boolean(estimate.historical_matches?.length)].filter(Boolean).length, 5);
  const materialScore = score01([materials.length > 0, pricedMaterials.length === materials.length && materials.length > 0, /model|brand|part|sku/i.test(text), Boolean(research.priceFindings?.length || estimate.research_metadata?.openai_live_search_used), materials.some((m) => /openai|internal|catalog|supplier|allowance/i.test(m.source || ''))].filter(Boolean).length, 5);
  const pricingScore = score01([pricedMaterials.length === materials.length && materials.length > 0, pricedLabor.length === labor.length && labor.length > 0, Boolean(research.priceFindings?.length || estimate.research_metadata?.openai_live_search_used), Boolean(estimate.pricing_summary?.grand_total_cents), !estimate.pricing_warnings?.length].filter(Boolean).length, 5);
  const researchScore = score01([materials.length > 0, Boolean(estimate.research_metadata?.openai_live_search_used), Boolean(research.priceFindings?.length), Boolean(research.productFindings?.length)].filter(Boolean).length, 4);
  ...
  const reasons = [`${scores.level} confidence: scope ${Math.round(scores.scope_completeness * 100)}%, photos ${Math.round(scores.photo_analysis * 100)}%, trade ${Math.round(scores.trade_certainty * 100)}%, materials ${Math.round(scores.material_certainty * 100)}%, pricing ${Math.round(scores.pricing_certainty * 100)}%, regional ${Math.round(scores.regional_certainty * 100)}%, data ${Math.round(scores.data_completeness * 100)}%.`];
  if (pricedMaterials.length !== materials.length) reasons.push('One or more material lines required allowance pricing because verified unit cost was missing.');
  if (pricedLabor.length !== labor.length) reasons.push('One or more labor lines required default-rate backfill.');
  if (!estimate.research_metadata?.openai_live_search_used && !research.priceFindings?.length) reasons.push('Live pricing evidence was not available; internal allowances lowered pricing confidence.');
  ...
  return { scores, reasons, recommended_action, level: scores.level, explanation: reasons[0] };
};
