...
const DEFAULT_PORTAL_SETTINGS = {
  ...
  aiPhotoEstimate: { title: 'AI Photo Estimate', status: 'Demo intake ready', bullets: ['✓ Upload photos','✓ AI analyzes project','✓ Receive estimate','✓ Approve online'] },
...
export const DEFAULT_HOMEPAGE_SETTINGS = {
  ...
  footerText: 'Customer-focused repair, maintenance, installation, punch-list, and property improvement help.', footerPhone: '', footerEmail: '', footerAddress: '', socialLinks: {}, licenseText: '', sectionVisibility: DEFAULT_VISIBILITY, portal: DEFAULT_PORTAL_SETTINGS,
};
...
const normalizePortalSettings = (portal = {}) => {
  const source = typeof portal === 'object' && portal && !Array.isArray(portal) ? portal : {};
  const defaults = DEFAULT_PORTAL_SETTINGS;
  return {
    ...defaults,
    ...source,
    badge: clean(source.badge || defaults.badge, 120),
    headline: clean(source.headline || defaults.headline, 160),
    subheadline: clean(source.subheadline || defaults.subheadline, 240),
    description: clean(source.description || defaults.description, 700),
    features: Array.isArray(source.features) && source.features.length ? source.features.map((item) => clean(item, 140)).filter(Boolean) : defaults.features,
    previewCards: Array.isArray(source.previewCards) && source.previewCards.length ? source.previewCards.slice(0, 6).map((card) => ({ title: clean(card?.title, 80), status: clean(card?.status, 100), description: clean(card?.description || card?.subtitle, 160) })).filter((card) => card.title || card.status || card.description) : defaults.previewCards,
    aiPhotoEstimate: { ...defaults.aiPhotoEstimate, ...(typeof source.aiPhotoEstimate === 'object' && source.aiPhotoEstimate && !Array.isArray(source.aiPhotoEstimate) ? source.aiPhotoEstimate : {}), bullets: Array.isArray(source.aiPhotoEstimate?.bullets) && source.aiPhotoEstimate.bullets.length ? source.aiPhotoEstimate.bullets.map((item) => clean(item, 140)).filter(Boolean) : defaults.aiPhotoEstimate.bullets },
    loginEyebrow: clean(source.loginEyebrow || defaults.loginEyebrow, 80),
    loginTitle: clean(source.loginTitle || defaults.loginTitle, 160),
    loginCopy: clean(source.loginCopy || defaults.loginCopy, 240),
    submitLabel: clean(source.submitLabel || defaults.submitLabel, 80),
    trustItems: Array.isArray(source.trustItems) && source.trustItems.length ? source.trustItems.map((item) => clean(item, 140)).filter(Boolean) : defaults.trustItems,
    trustMessage: clean(source.trustMessage || defaults.trustMessage, 300),
    estimatePrompt: clean(source.estimatePrompt || defaults.estimatePrompt, 120),
    requestEstimateLabel: clean(source.requestEstimateLabel || defaults.requestEstimateLabel, 80),
    requestEstimateLink: clean(source.requestEstimateLink || defaults.requestEstimateLink, 300),
    backgroundImage: clean(source.backgroundImage, 1000),
    accentColor: clean(source.accentColor, 80),
  };
};
