...
const ensureStructuredQuoteLines = ({ quote = {}, laborLineItems = [], materialLineItems = [], otherPricing = {}, targetGrandCents = 0 }) => {
  ...
  if (desiredGrand > 0 && !labor.length && !materials.length) {
    ...
    labor = [{ name: 'Service labor / site prep / installation', description: 'AI generated allowance line for admin review.', hours: laborHours, quantity: laborHours, unit: 'hours', rate: dollars(backfillRateCents), rate_cents: backfillRateCents, unitCostCents: backfillRateCents, unit_cost_cents: backfillRateCents, total: dollars(laborCents), totalCents: laborCents, total_cents: laborCents, confidence: 'low', source: 'ai_allowance_backfill', backfilled: true }];
    materials = [{ name: 'Materials, fasteners, and consumables allowance', description: 'AI generated materials allowance for admin review.', quantity: 1, unit: 'allowance', unit_cost: dollars(materialCents), unitCostCents: materialCents, unit_cost_cents: materialCents, markup_percent: 0, markupPct: 0, total: dollars(materialCents), totalCents: materialCents, total_cents: materialCents, source: 'ai_allowance_backfill', source_url: '', last_checked: new Date().toISOString().slice(0, 10), confidence: 'low', backfilled: true }];
    warnings.push('Line items were completed from the AI pricing total for admin review.');
  }
  ...
  if (desiredGrand > 0 && Math.abs(summary.grand_total_cents - desiredGrand) > 1 && desiredGrand > summary.grand_total_cents) {
    ...
    materials.push({ name: 'Pricing allowance adjustment', description: 'AI generated allowance line for admin review.', quantity: 1, unit: 'allowance', unit_cost: dollars(delta), unitCostCents: delta, unit_cost_cents: delta, markup_percent: 0, markupPct: 0, total: dollars(delta), totalCents: delta, total_cents: delta, source: 'ai_allowance_backfill', backfilled: true, source_url: '', last_checked: new Date().toISOString().slice(0, 10), confidence: 'low' });
...
export default async (request) => {
  ...
  try {
      ...
      return json(200, {
        ...
        draft: {
          ...
          structuredEstimate: {
            ...
            research_metadata: { research_mode: 'openai_first', openai_live_search_used: Boolean(aiFirstQuote.materialResearchContext?.openAiLiveSearchAvailable || aiFirstQuote.livePricingVerification?.openAiLiveSearchAvailable), openai_live_search_requested: Boolean(aiFirstQuote.materialResearchContext?.openAiLiveSearchAttempted || aiFirstQuote.livePricingVerification?.openAiLiveSearchAttempted), openai_live_search_unavailable: Boolean((aiFirstQuote.materialResearchContext?.openAiLiveSearchAttempted || aiFirstQuote.livePricingVerification?.openAiLiveSearchAttempted) && !(aiFirstQuote.materialResearchContext?.openAiLiveSearchAvailable || aiFirstQuote.livePricingVerification?.openAiLiveSearchAvailable)), fallback_search_used: false, responses_api_used: true, openai_model: aiFirstQuote.model || OPENAI_MODEL || 'gpt-5.5', internal_catalog_used: true, historical_quotes_used: Boolean(aiFirstQuote.historicalMatchUsed), serpapi_used: false, sources: normalizedMaterialLines.map((item) => ({ title: item.name, source: item.source, url: item.source_url })), pricing_confidence_reason: aiFirstQuote.pricingConfidenceReason || 'OpenAI Responses API pricing with internal catalog and historical context.' },
    ...
    const summaryLines = [
      ...
      '',
      'Confirm exact part quantities, tax/shipping, and final labor scope.',
    ];
