...
// HOTFIX VERSION:
// - Prevents 504 by using a short OpenAI timeout
// - Uses local estimator fallback immediately if OpenAI is slow/fails
// - Limits SerpApi live price lookups to 3 by default
// - Never auto-sends quotes

const DEFAULT_MODEL = 'gpt-5-mini';
const OPENAI_TIMEOUT_MS = Number(process.env.AI_OPENAI_TIMEOUT_MS || 12000);
const SERPAPI_TIMEOUT_MS = Number(process.env.AI_SERPAPI_TIMEOUT_MS || 3500);
const MAX_LIVE_PRICE_LOOKUPS = Number(process.env.AI_MAX_LIVE_PRICE_LOOKUPS || 3);
const DISABLE_LIVE_PRICING = String(process.env.AI_DISABLE_LIVE_PRICING || '').toLowerCase() === 'true';
...
function getPricingSettings(payload = {}) {
  return {
...
function deriveMissingQuestions(req) {
  const text = slug(`${req.workScope} ${req.service} ${req.subcategory} ${req.description}`);
  const questions = [];
  if (!req.workScope) questions.push('Is this troubleshooting/repair, replacing existing, or a new install?');
  if (!req.service) questions.push('What type of work is this: HVAC, electrical, plumbing, carpentry, drywall, painting, appliance/fixture, flooring, exterior, or general maintenance?');
  if (!req.streetAddress) questions.push('What is the property address for the work?');
  if (!req.description || req.description.length < 35) questions.push('Please describe what needs done, what is wrong, and what result you want.');
  if (!req.photosProvided) questions.push('Can you upload clear photos of the work area and existing item?');
  if (!/(model|brand|size|btu|amp|volt|gallon|inch|measurement|ft|feet|linear|sq)/i.test(text)) {
    questions.push('Do you have brand, model, size, voltage, BTU, measurements, or part information?');
  }
  return questions.slice(0, 8);
}

function detectFlags(req) {
  const text = slug(`${req.workScope} ${req.service} ${req.subcategory} ${req.description}`);
  const licensed = [];
  const permit = [];
  const risk = [];

  if (/panel|breaker|new circuit|dedicated circuit|disconnect|meter|service upgrade/.test(text)) {
    licensed.push('Electrical panel/new circuit/disconnect scope may require a licensed electrician.');
    permit.push('Electrical permit/code review may be required.');
  }
  if (/gas|natural gas|propane/.test(text)) licensed.push('Gas work requires licensed trade review.');
  if (/refrigerant|line set|mini split|mini-split|ductless|condenser|hvac/.test(text)) {
    licensed.push('HVAC/refrigerant/startup scope may require licensed HVAC review.');
    permit.push('HVAC permit/inspection may be required depending on local code.');
  }
...
function materialSeeds(req) {
  const text = slug(`${req.workScope} ${req.service} ${req.subcategory} ${req.description}`);
  const items = [];
  const add = (name, query, qty = 1, low = 0, high = 0, notes = '') => {
    items.push({ name, search_query: query || name, quantity: qty, required: true, estimated_cost_low: low, estimated_cost_high: high, notes });
  };
  ...
  if (/mini split|mini-split|ductless/.test(text)) {
    ...
    add('Toilet', 'elongated toilet complete kit', 1, 120, 450);
    add('Wax ring', 'extra thick toilet wax ring with flange', 1, 5, 15);
    add('Closet bolts', 'toilet closet bolt kit', 1, 4, 12);
    add('Supply line', 'toilet supply line 3/8 compression', 1, 8, 25);
    add('Shutoff valve allowance', 'toilet angle stop shutoff valve', 1, 9, 35);
    add('Caulk', 'bath kitchen silicone caulk white', 1, 5, 18);
  } else if (/outlet|gfci|switch|dimmer|light fixture|ceiling fan|electrical/.test(text)) {
    add('Device or fixture allowance', 'electrical switch outlet light fixture ceiling fan', 1, 8, 200);
    add('Cover plate', 'decorator wall plate white', 1, 2, 12);
    add('Wire connectors', 'wire nuts electrical connector assortment', 1, 5, 25);
    add('Electrical box extender or repair parts', 'electrical box extender outlet switch', 1, 4, 25);
    add('Fasteners and electrical tape', 'electrical tape machine screws grounding pigtail', 1, 5, 25);
  } else if (/drywall|patch|hole|texture/.test(text)) {
    add('Drywall patch materials', 'drywall patch repair kit', 1, 10, 55);
    add('Joint compound', 'joint compound small tub', 1, 8, 30);
    add('Drywall tape', 'drywall tape roll', 1, 4, 15);
    add('Texture match material', 'wall texture spray orange peel knockdown', 1, 12, 35);
    add('Primer and paint allowance', 'interior primer paint touch up', 1, 20, 100);
  } else {
...
function laborSeeds(req) {
  ...
  if (/troubleshoot|repair|not working|leak|clog|trip|tripping|noise/.test(text)) {
    base[2] = { name: 'Diagnosis and repair attempt', description: 'Troubleshoot issue, identify failed parts/conditions, perform approved repair if possible.', low_hours: 1, high_hours: 4, skill_level: 'handyman', notes: 'Final repair depends on diagnosis.' };
  }

  if (/replace|replacement/.test(text)) {
    base.splice(2, 0, { name: 'Remove existing item', description: 'Disconnect/remove old item and prepare area for replacement.', low_hours: 0.25, high_hours: 1.5, skill_level: 'handyman', notes: 'Corrosion, damage, or stuck fasteners add time.' });
  }

  if (/new install|install/.test(text)) {
    base[2] = { name: 'New install', description: 'Layout, mount, connect, secure, seal, and test the new item.', low_hours: 1.25, high_hours: 4.5, skill_level: 'handyman', notes: 'No existing utilities/support may require extra work.' };
  }
...
function outputSchema() {
  return {
    ...
    material_search_items: [{ name:'string', search_query:'string', quantity:'number', required:'boolean', notes:'string' }],
    ...
    risk_flags: ['string'],
    licensed_trade_flags: ['string'],
    permit_flags: ['string'],
    exclusions: ['string'],
    customer_responsibilities: ['string'],
    technician_checklist: ['string'],
    change_order_triggers: ['string'],
    customer_facing_quote: 'string',
    internal_technician_notes: 'string'
  };
}

async function callOpenAi(req, price) {
        ...
        input: [
          { role: 'system', content: 'You are a senior handyman estimator for T&A Contracting. Return strict valid JSON only. No markdown. Do not auto-send or final-approve quotes.' },
          {
            ...
            content: JSON.stringify({
              task: 'Create a complete AI handyman quote draft for admin review.',
              request: req,
              pricing_settings: price,
              rules: [
                'Think like an experienced handyman estimator.',
                'Ask missing questions if incomplete.',
                'Break labor into real phases.',
                'Include hidden materials, fittings, fasteners, sealants, consumables, disposal, finish work, access, safety.',
                'Flag unsafe/licensed/permitted work.',
                'If important info is missing, set is_quote_ready=false.',
                'Return material_search_items as Google Shopping search queries.',
                'Do not invent exact law/code claims; use verify local code.'
              ],
...
function normalizeDraft(ai, req) {
  const seedMats = materialSeeds(req);
  const seedLabor = laborSeeds(req);
  const miss = deriveMissingQuestions(req);
  const fl = detectFlags(req);
  const draft = ai && typeof ai === 'object' ? ai : {};

  const matSearch = Array.isArray(draft.material_search_items) && draft.material_search_items.length
    ? draft.material_search_items
    : seedMats.map((m) => ({ name: m.name, search_query: m.search_query, quantity: m.quantity, required: true, notes: m.notes || '' }));

  const mats = Array.isArray(draft.materials) && draft.materials.length
    ? draft.materials
    : seedMats.map((m) => ({
      name: m.name,
      quantity: m.quantity,
...
async function fetchSerpApiPrice(item) {
  const apiKey = process.env.SERPAPI_API_KEY;
  if (!apiKey || DISABLE_LIVE_PRICING) return null;
  ...
  try {
    ...
    url.searchParams.set('q', item.search_query || item.name);
    ...
    const products = (data.shopping_results || [])
      .map((p) => ({
        title: clean(p.title, 220),
        source: clean(p.source, 120),
        link: clean(p.link || p.product_link, 1000),
        price_text: clean(p.price, 80),
        price: Number(p.extracted_price || String(p.price || '').replace(/[^0-9.]/g, '')),
        delivery: clean(p.delivery, 160),
        rating: p.rating || null,
        reviews: p.reviews || null,
        thumbnail: clean(p.thumbnail, 1000),
      }))
...
async function livePrices(draft) {
  const searchItems = (draft.material_search_items || []).slice(0, MAX_LIVE_PRICE_LOOKUPS);
  const results = await Promise.all(searchItems.map(async (item) => {
    const result = await fetchSerpApiPrice(item);
    return {
      name: clean(item.name, 180),
      search_query: clean(item.search_query || item.name, 240),
      quantity: Number(item.quantity || 1),
      required: Boolean(item.required),
      notes: clean(item.notes, 500),
      ...(result || { live_pricing_available: false, selected_unit_price: null, unit_price_low: null, unit_price_high: null, products: [] }),
    };
  }));
  return results;
}

function calculateTotals(draft, live, price) {
  const labor_items = draft.labor_items.map((item) => {
    const low = Math.max(0, Number(item.low_hours || 0));
    const high = Math.max(low, Number(item.high_hours || low || 0));
    return {
      name: clean(item.name, 180),
      description: clean(item.description, 900),
      low_hours: money(low),
      high_hours: money(high),
      skill_level: clean(item.skill_level, 120),
      notes: clean(item.notes, 900)
    };
  });

  const labor_hours_low = labor_items.reduce((s, x) => s + x.low_hours, 0);
  const labor_hours_high = labor_items.reduce((s, x) => s + x.high_hours, 0);
  const labor_cost_low = labor_hours_low * price.laborRate;
  const labor_cost_high = labor_hours_high * price.laborRate;

  const materials = draft.materials.map((m) => {
    const qty = Math.max(1, Number(m.quantity || 1));
    const lp = live.find((p) => slug(p.name) === slug(m.name) || slug(p.name).includes(slug(m.name)) || slug(m.name).includes(slug(p.name)));
    const unitLow = Number(lp?.unit_price_low ?? m.estimated_cost_low ?? 0);
    const unitHigh = Number(lp?.unit_price_high ?? m.estimated_cost_high ?? unitLow);
    return {
      name: clean(m.name, 180),
      quantity: qty,
      required: Boolean(m.required),
      customer_supplied: Boolean(m.customer_supplied),
      estimated_cost_low: money(unitLow),
      estimated_cost_high: money(unitHigh),
      total_cost_low: money(unitLow * qty),
      total_cost_high: money(unitHigh * qty),
      live_pricing_available: Boolean(lp?.live_pricing_available),
      live_products: lp?.products || [],
      notes: clean(m.notes, 900)
    };
  });

  const material_cost_low = materials.reduce((s, m) => s + (m.customer_supplied ? 0 : m.total_cost_low), 0);
  const material_cost_high = materials.reduce((s, m) => s + (m.customer_supplied ? 0 : m.total_cost_high), 0);
  const material_markup_low = material_cost_low * (price.materialMarkupPercent / 100);
  const material_markup_high = material_cost_high * (price.materialMarkupPercent / 100);
  const subtotalLow = labor_cost_low + material_cost_low + material_markup_low + price.tripCharge;
  const subtotalHigh = labor_cost_high + material_cost_high + material_markup_high + price.tripCharge;
  const contingency_low = subtotalLow * (price.contingencyPercent / 100);
  const contingency_high = subtotalHigh * (price.contingencyPercent / 100);

  return {
    labor_items,
    materials,
    labor_hours_low: money(labor_hours_low),
    labor_hours_high: money(labor_hours_high),
    labor_cost_low: money(labor_cost_low),
    labor_cost_high: money(labor_cost_high),
    material_cost_low: money(material_cost_low),
    material_cost_high: money(material_cost_high),
    material_markup_low: money(material_markup_low),
    material_markup_high: money(material_markup_high),
    trip_charge: money(price.tripCharge),
    contingency_low: money(contingency_low),
    contingency_high: money(contingency_high),
    total_low: money(Math.max(price.minimumCharge, subtotalLow + contingency_low)),
    total_high: money(Math.max(price.minimumCharge, subtotalHigh + contingency_high))
  };
}

function customerQuote(draft, t) {
  return [
    draft.customer_facing_quote,
    '',
    `Estimated labor: ${t.labor_hours_low}–${t.labor_hours_high} hours`,
    `Estimated labor cost: $${t.labor_cost_low}–$${t.labor_cost_high}`,
    `Estimated materials before markup: $${t.material_cost_low}–$${t.material_cost_high}`,
    `Estimated total: $${t.total_low}–$${t.total_high}`,
    '',
    'This is an AI-assisted draft and must be reviewed/approved by T&A Contracting before it is sent as a final quote.'
  ].join('\n').trim();
}
...
export const handler = async (event) => {
  ...
  if (!request.description && !request.service && !request.workScope) {
    return json(400, { ok: false, message: 'Missing description, service, or workScope' });
  }

  const price = getPricingSettings(input);
  const aiRaw = await callOpenAi(request, price);
  const draft = normalizeDraft(aiRaw, request);
  const live_material_prices = await livePrices(draft);
  const t = calculateTotals(draft, live_material_prices, price);

  return json(200, {
    ok: true,
    source: aiRaw ? 'openai' : 'local-fallback-fast',
    timeoutProtected: true,
    createdAt: new Date().toISOString(),
    request,
    quote_ready: draft.is_quote_ready,
    job_summary: draft.job_summary,
    category: draft.category,
    subcategory: draft.subcategory,
    work_scope: draft.work_scope,
    confidence: draft.confidence,
    missing_required_info: draft.missing_required_info,
    questions_to_customer: draft.questions_to_customer,
    assumptions: draft.assumptions,
    labor_items: t.labor_items,
    materials: t.materials,
    live_material_prices,
    difficulty_factors: draft.difficulty_factors,
    risk_flags: draft.risk_flags,
    licensed_trade_flags: draft.licensed_trade_flags,
    permit_flags: draft.permit_flags,
    exclusions: draft.exclusions,
    customer_responsibilities: draft.customer_responsibilities,
    technician_checklist: draft.technician_checklist,
    change_order_triggers: draft.change_order_triggers,
    internal_technician_notes: draft.internal_technician_notes,
    customer_facing_quote: customerQuote(draft, t),
    totals: t,
    pricing_settings_used: price,
    admin_note: 'Save as draft quote. Admin must review/edit before sending.'
  });
};
