...
const camel = (row = {}) => ({
  ...
  sidebarBackgroundColor: row.sidebar_background_color || FALLBACK_COMPANY.sidebarBackgroundColor,
  sidebarTextColor: row.sidebar_text_color || FALLBACK_COMPANY.sidebarTextColor,
  sidebarActiveColor: row.sidebar_active_color || FALLBACK_COMPANY.sidebarActiveColor,
  sidebarBorderColor: row.sidebar_border_color || FALLBACK_COMPANY.sidebarBorderColor,
  sidebarHoverColor: row.sidebar_hover_color || FALLBACK_COMPANY.sidebarHoverColor,
  mobileNavBackgroundColor: row.mobile_nav_background_color || FALLBACK_COMPANY.mobileNavBackgroundColor,
  mobileNavTextColor: row.mobile_nav_text_color || row.sidebar_text_color || FALLBACK_COMPANY.mobileNavTextColor,
  mobileNavActiveColor: row.mobile_nav_active_color || FALLBACK_COMPANY.mobileNavActiveColor,
  mobileNavBorderColor: row.mobile_nav_border_color || row.sidebar_border_color || FALLBACK_COMPANY.mobileNavBorderColor,
  customSidebarColorsEnabled: Boolean(row.custom_sidebar_colors_enabled),
  customMobileNavColorsEnabled: Boolean(row.custom_mobile_nav_colors_enabled),
  hasCustomSidebarColors: Boolean(row.custom_sidebar_colors_enabled),
  ...
  selectedTheme: row.selected_theme || row.theme_mode || row.default_theme || FALLBACK_COMPANY.selectedTheme,
  colorScheme: row.color_scheme || row.theme_mode || row.default_theme || FALLBACK_COMPANY.colorScheme,
...
export const ensureCompanyTables = async (db) => {
  ...
  await db.sql`alter table company_settings add column if not exists selected_theme text`;
  await db.sql`alter table company_settings add column if not exists color_scheme text`;
  await db.sql`alter table company_settings add column if not exists sidebar_background_color text`;
  await db.sql`alter table company_settings add column if not exists sidebar_text_color text`;
  await db.sql`alter table company_settings add column if not exists sidebar_active_color text`;
  await db.sql`alter table company_settings add column if not exists sidebar_border_color text`;
  await db.sql`alter table company_settings add column if not exists sidebar_hover_color text`;
  await db.sql`alter table company_settings add column if not exists mobile_nav_background_color text`;
  await db.sql`alter table company_settings add column if not exists mobile_nav_active_color text`;
  await db.sql`alter table company_settings add column if not exists mobile_nav_text_color text`;
  await db.sql`alter table company_settings add column if not exists mobile_nav_border_color text`;
  await db.sql`alter table company_settings add column if not exists custom_sidebar_colors_enabled boolean not null default false`;
  await db.sql`alter table company_settings add column if not exists custom_mobile_nav_colors_enabled boolean not null default false`;
...
const payloadFrom = (body) => ({
  companyName: clean(body.companyName || body.company_name, 160) || 'Your Company', legalName: clean(body.legalName || body.legal_name, 180), displayName: clean(body.displayName || body.display_name, 160), websiteUrl: clean(body.websiteUrl || body.website_url, 300), supportEmail: clean(body.supportEmail || body.support_email, 254), supportPhone: clean(body.supportPhone || body.support_phone, 60), businessPhone: clean(body.businessPhone || body.business_phone, 60), businessAddress: clean(body.businessAddress || body.business_address, 300), city: clean(body.city, 120), state: clean(body.state, 60), zip: clean(body.zip, 40), serviceArea: clean(body.serviceArea || body.service_area, 500), timezone: clean(body.timezone, 80) || 'America/Phoenix', currency: clean(body.currency, 8) || 'USD', licenseNumber: clean(body.licenseNumber || body.license_number, 120), contractorLicenseNumber: clean(body.contractorLicenseNumber || body.contractor_license_number, 120), taxId: clean(body.taxId || body.tax_id, 120), businessType: clean(body.businessType || body.business_type, 120), logoUrl: clean(body.logoUrl || body.logo_url, 700), faviconUrl: clean(body.faviconUrl || body.favicon_url, 700), primaryColor: clean(body.primaryColor || body.primary_color, 32) || '#2563eb', accentColor: clean(body.accentColor || body.accent_color, 32) || '#22c55e', backgroundColor: clean(body.backgroundColor || body.background_color, 32) || '#f8fafc', surfaceColor: clean(body.surfaceColor || body.surface_color, 32) || '#ffffff', textColor: clean(body.textColor || body.text_color, 32) || '#0f172a', buttonColor: clean(body.buttonColor || body.button_color, 32) || clean(body.primaryColor || body.primary_color, 32) || '#2563eb', successColor: clean(body.successColor || body.success_color, 32) || '#16a34a', warningColor: clean(body.warningColor || body.warning_color, 32) || '#f59e0b', dangerColor: clean(body.dangerColor || body.danger_color, 32) || '#dc2626', sidebarBackgroundColor: clean(body.sidebarBackgroundColor || body.sidebar_background_color, 32), sidebarTextColor: clean(body.sidebarTextColor || body.sidebar_text_color, 32), sidebarActiveColor: clean(body.sidebarActiveColor || body.sidebar_active_color, 32), sidebarBorderColor: clean(body.sidebarBorderColor || body.sidebar_border_color, 32), sidebarHoverColor: clean(body.sidebarHoverColor || body.sidebar_hover_color, 32), mobileNavBackgroundColor: clean(body.mobileNavBackgroundColor || body.mobile_nav_background_color, 32), mobileNavTextColor: clean(body.mobileNavTextColor || body.mobile_nav_text_color, 32), mobileNavActiveColor: clean(body.mobileNavActiveColor || body.mobile_nav_active_color, 32), mobileNavBorderColor: clean(body.mobileNavBorderColor || body.mobile_nav_border_color, 32), customSidebarColorsEnabled: boolSetting(body.customSidebarColorsEnabled ?? body.custom_sidebar_colors_enabled, false), customMobileNavColorsEnabled: boolSetting(body.customMobileNavColorsEnabled ?? body.custom_mobile_nav_colors_enabled, false), themeMode: clean(body.themeMode || body.theme_mode, 20) || clean(body.defaultTheme || body.default_theme, 20) || 'system', defaultTheme: clean(body.defaultTheme || body.default_theme, 20) || clean(body.themeMode || body.theme_mode, 20) || 'system', selectedTheme: clean(body.selectedTheme || body.selected_theme, 20) || clean(body.themeMode || body.theme_mode, 20) || 'system', colorScheme: clean(body.colorScheme || body.color_scheme, 20) || clean(body.themeMode || body.theme_mode, 20) || 'system', enableThemeToggle: boolSetting(body.enableThemeToggle ?? body.enable_theme_toggle, true), showCompanyNameInHeader: boolSetting(body.showCompanyNameInHeader ?? body.show_company_name_in_header, false),
...
export default async (request) => {
  ...
  try {
    ...
    const [row] = existing ? await db.sql`update company_settings set company_name=${p.companyName}, legal_name=${p.legalName || null}, display_name=${p.displayName || p.companyName}, website_url=${p.websiteUrl || null}, support_email=${p.supportEmail || null}, support_phone=${p.supportPhone || null}, business_phone=${p.businessPhone || null}, business_address=${p.businessAddress || null}, city=${p.city || null}, state=${p.state || null}, zip=${p.zip || null}, service_area=${p.serviceArea || null}, timezone=${p.timezone}, currency=${p.currency}, license_number=${p.licenseNumber || null}, contractor_license_number=${p.contractorLicenseNumber || null}, tax_id=${p.taxId || null}, business_type=${p.businessType || null}, logo_url=${p.logoUrl || null}, favicon_url=${p.faviconUrl || null}, primary_color=${p.primaryColor}, accent_color=${p.accentColor}, background_color=${p.backgroundColor}, surface_color=${p.surfaceColor}, text_color=${p.textColor}, button_color=${p.buttonColor}, success_color=${p.successColor}, warning_color=${p.warningColor}, danger_color=${p.dangerColor}, sidebar_background_color=${p.sidebarBackgroundColor || null}, sidebar_text_color=${p.sidebarTextColor || null}, sidebar_active_color=${p.sidebarActiveColor || null}, sidebar_border_color=${p.sidebarBorderColor || null}, sidebar_hover_color=${p.sidebarHoverColor || null}, mobile_nav_background_color=${p.mobileNavBackgroundColor || null}, mobile_nav_text_color=${p.mobileNavTextColor || null}, mobile_nav_active_color=${p.mobileNavActiveColor || null}, mobile_nav_border_color=${p.mobileNavBorderColor || null}, custom_sidebar_colors_enabled=${Boolean(p.customSidebarColorsEnabled)}, custom_mobile_nav_colors_enabled=${Boolean(p.customMobileNavColorsEnabled)}, selected_theme=${p.selectedTheme}, color_scheme=${p.colorScheme}, theme_mode=${p.themeMode}, default_theme=${p.defaultTheme}, enable_theme_toggle=${Boolean(p.enableThemeToggle)}, show_company_name_in_header=${p.showCompanyNameInHeader}, admin_settings=${JSON.stringify(p.adminSettings)}::jsonb, updated_at=now() where id=${existing.id} returning *`
      : await db.sql`insert into company_settings (company_name, legal_name, display_name, website_url, support_email, support_phone, business_phone, business_address, city, state, zip, service_area, timezone, currency, license_number, contractor_license_number, tax_id, business_type, logo_url, favicon_url, primary_color, accent_color, background_color, surface_color, text_color, button_color, success_color, warning_color, danger_color, sidebar_background_color, sidebar_text_color, sidebar_active_color, sidebar_border_color, sidebar_hover_color, mobile_nav_background_color, mobile_nav_text_color, mobile_nav_active_color, mobile_nav_border_color, custom_sidebar_colors_enabled, custom_mobile_nav_colors_enabled, selected_theme, color_scheme, theme_mode, default_theme, enable_theme_toggle, show_company_name_in_header, admin_settings) values (${p.companyName}, ${p.legalName || null}, ${p.displayName || p.companyName}, ${p.websiteUrl || null}, ${p.supportEmail || null}, ${p.supportPhone || null}, ${p.businessPhone || null}, ${p.businessAddress || null}, ${p.city || null}, ${p.state || null}, ${p.zip || null}, ${p.serviceArea || null}, ${p.timezone}, ${p.currency}, ${p.licenseNumber || null}, ${p.contractorLicenseNumber || null}, ${p.taxId || null}, ${p.businessType || null}, ${p.logoUrl || null}, ${p.faviconUrl || null}, ${p.primaryColor}, ${p.accentColor}, ${p.backgroundColor}, ${p.surfaceColor}, ${p.textColor}, ${p.buttonColor}, ${p.successColor}, ${p.warningColor}, ${p.dangerColor}, ${p.sidebarBackgroundColor || null}, ${p.sidebarTextColor || null}, ${p.sidebarActiveColor || null}, ${p.sidebarBorderColor || null}, ${p.sidebarHoverColor || null}, ${p.mobileNavBackgroundColor || null}, ${p.mobileNavTextColor || null}, ${p.mobileNavActiveColor || null}, ${p.mobileNavBorderColor || null}, ${Boolean(p.customSidebarColorsEnabled)}, ${Boolean(p.customMobileNavColorsEnabled)}, ${p.selectedTheme}, ${p.colorScheme}, ${p.themeMode}, ${p.defaultTheme}, ${Boolean(p.enableThemeToggle)}, ${p.showCompanyNameInHeader}, ${JSON.stringify(p.adminSettings)}::jsonb) returning *`;
