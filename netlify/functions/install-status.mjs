...
export default async (request) => {
  ...
  try {
    ...
    const [company] = await db.sql`select installation_complete, license_status, license_validation_enabled, license_verified_at, license_verification_provider from company_settings order by created_at asc limit 1`;
    const [install] = await db.sql`select installed, installer_locked, license_status, license_validation_enabled, license_verified_at, license_verification_provider from platform_install order by created_at desc limit 1`;
    ...
    return json(200, { ok: true, installed, installer_locked: Boolean(install?.installer_locked || installed), company_settings_exists: companySettingsExists, owner_exists: ownerExists, roles_exist: rolesExist, permissions_exist: permissionsExist, company_installation_complete: companyComplete, platform_install_installed: Boolean(install?.installed), license: { status: company?.license_status || install?.license_status || 'not_configured', validationEnabled: Boolean(company?.license_validation_enabled || install?.license_validation_enabled), verifiedAt: company?.license_verified_at || install?.license_verified_at || null, provider: company?.license_verification_provider || install?.license_verification_provider || null } });
  } catch (error) {
