...
const databaseCheckingMessage='Database not detected. Attempting automatic bootstrap...';
...
function dbNotConfigured(message=databaseCheckingMessage, extra={}){ return {ok:false,installed:false,needsInstall:true,safeMode:true,code:'DATABASE_NOT_CONFIGURED',message,canRetry:true,manualSetupRequired:false,netlifyDatabaseDetected:false,...extra}; }
function dbManualRequired(message='Automatic creation is not available from this deployed function. Link a Netlify Database, then click Retry.', extra={}){ return {ok:false,installed:false,needsInstall:true,safeMode:true,code:'DATABASE_MANUAL_LINK_REQUIRED',message,canRetry:true,manualSetupRequired:true,netlifyDatabaseDetected:false,guide:netlifyDatabaseGuide,...extra}; }
function dbUnavailable(message=safeDatabaseMessage, extra={}){ return {ok:false,installed:false,needsInstall:true,safeMode:true,code:'DATABASE_DRIVER_LOAD_FAILED',message,canRetry:true,manualSetupRequired:false,netlifyDatabaseDetected:true,...extra}; }
...
function isDatabaseError(error){ return ['DATABASE_NOT_CONFIGURED','DATABASE_MANUAL_LINK_REQUIRED','DATABASE_UNAVAILABLE','DATABASE_DRIVER_LOAD_FAILED','DATABASE_DRIVER_MISSING','NETLIFY_DATABASE_REQUIRED'].includes(error?.code)||/database|postgres|connection|pg|relation|schema|migration|module|import|package/i.test(error?.message||''); }
...
async function tryInstallDb(fn, failureBody){
 try{ return await withDb(fn); }catch(error){
  if(['DATABASE_NOT_CONFIGURED','NETLIFY_DATABASE_REQUIRED'].includes(error?.code)) return json(200,{...dbNotConfigured(undefined,{draft:failureBody.draft,goToStep:failureBody.goToStep}),databaseError:error.message});
...
async function databaseHealth(){
 ...
 const { ensureSchema, sql, getDatabaseUrl, databaseEnvStatus, databaseDriverPackage, loadDatabaseDriverResult }=deps;
 ...
 if(!configured) return {configured:false,reachable:false,driverPackage:databaseDriverPackage,driverLoaded,driverError:null,error:`Database is not configured yet. Checked: ${databaseEnvStatus().map((item)=>item.key).join(', ')}.`,env:databaseEnvStatus(),requiresDatabase:true,manualSetupRequired:false};
 if(!driverLoaded) return {configured:true,reachable:false,driverPackage:databaseDriverPackage,driverLoaded,driverError,error:driverError,env:databaseEnvStatus(),code:'DATABASE_DRIVER_LOAD_FAILED'};
 try{
  ...
  return {configured:true,reachable:false,driverPackage:databaseDriverPackage,driverLoaded,driverError:null,error:error.message,env:databaseEnvStatus(),code:error?.code};
 }
}
...
export const handler=async(event)=>{ try{
 ...
 if(path==='/install-status') {
  ...
  return json(200,{ok:true,installed:complete,needsInstall:!complete,installation_complete:complete,draft:rows[0]?.installer_draft||{},completed_at:rows[0]?.completed_at||null,databaseConfigured:true,databaseReachable:true,database:'ok',netlifyDatabaseDetected:true,driverPackage:health.driverPackage,driverLoaded:health.driverLoaded,env:health.env,migrations:health.migrations});
 }
 if(path==='/install/health') {
  const health=await databaseHealth();
  const body=health.reachable?{ok:true,code:'DATABASE_AVAILABLE',message:'Database ready.',database:'ok'}:(health.requiresDatabase?dbNotConfigured('Database not detected. Attempting automatic bootstrap...'):dbUnavailable());
  return json(200,{...body,safeMode:!health.reachable,databaseConfigured:health.configured,databaseReachable:health.reachable,netlifyDatabaseDetected:health.configured,databaseError:health.error||null,driverPackage:health.driverPackage,driverLoaded:health.driverLoaded,driverError:health.driverError,env:health.env,guide:body.manualSetupRequired?netlifyDatabaseGuide:undefined,needsInstall:!health.reachable,migrations:health.migrations});
 }
 if(path==='/install/bootstrap-database' && method==='POST') {
  const health=await databaseHealth();
  if(health.reachable) return json(200,{ok:true,code:'DATABASE_BOOTSTRAPPED',message:'Database ready.',database:'ok',databaseConfigured:true,databaseReachable:true,migrations:health.migrations});
  if(health.requiresDatabase) return json(200,{...dbManualRequired(),databaseConfigured:false,databaseReachable:false,databaseError:health.error,env:health.env,attemptedAutomaticBootstrap:true});
  return json(200,{...dbUnavailable(),databaseConfigured:health.configured,databaseReachable:false,databaseError:health.error,driverLoaded:health.driverLoaded,driverError:health.driverError,env:health.env,attemptedAutomaticBootstrap:true});
 }
 ...
 }catch(err){ const path=pathOf(event); if(path.startsWith('/install')||path==='/install-status') return json(200,{ok:false,code:['DATABASE_NOT_CONFIGURED','NETLIFY_DATABASE_REQUIRED'].includes(err?.code)?'DATABASE_NOT_CONFIGURED':(isDatabaseError(err)?'DATABASE_DRIVER_LOAD_FAILED':'INSTALL_API_ERROR'),message:['DATABASE_NOT_CONFIGURED','NETLIFY_DATABASE_REQUIRED'].includes(err?.code)?databaseCheckingMessage:(isDatabaseError(err)?safeDatabaseMessage:(err.message||'Unexpected installer API error')),safeMode:true,canRetry:true,manualSetupRequired:false,databaseError:isDatabaseError(err)?err.message:undefined}); return json(err.statusCode||500,{ok:false,error:err.message||'Unexpected error'}); }};
